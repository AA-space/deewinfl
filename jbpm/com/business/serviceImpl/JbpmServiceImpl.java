...
import com.business.dao.JbpmDao;
import com.business.entity.ActivityDetail;
import com.business.entity.DictionaryData;
import com.business.entity.GrantDelegateUser;
import com.business.entity.JBPMWorkflowHistoryInfo;
import com.business.entity.JBPMWorkflowHistoryInfoUser;
import com.business.entity.JBPMWorkflowReadUser;
import com.business.entity.JBPMWorkflowSignatureUser;
import com.business.entity.JbpmWorkflowDesigner;
import com.business.entity.User;
import com.business.entity.UserWorkflowStart;
import com.business.entity.UserWorkflowView;
import com.business.entity.WorkflowDesignerReject;
import com.business.entity.WorkflowStartDepartment;
import com.business.entity.WorkflowStartDepartmentRole;
import com.business.entity.WorkflowStartGroup;
import com.business.entity.WorkflowViewDepartment;
import com.business.entity.WorkflowViewDepartmentRole;
import com.business.entity.WorkflowViewGroup;
import com.business.exception.JbpmException;
import com.business.model.ActivityTaskUsers;
import com.business.model.ActivityTaskUsersComparator;
import com.business.model.ConditionRouteTypeEnum;
import com.business.model.DesignerActivity;
import com.business.model.JdbcModel;
import com.business.model.MessageTypeEnum;
import com.business.model.RouteInfo;
import com.business.model.SubmitTypeEnum;
import com.business.model.TaskUsers;
import com.business.model.WorkflowSubmitTextEnum;
import com.business.service.JbpmService;
import com.business.workflowRejectCondtion.RejectCondition;
import com.business.workflowRejectCondtion.RejectInfo;
import com.email.SendEmailUtil;
import com.jbpm.utils.DesignerTransferUtil;
import com.jbpm.utils.DraftUUIDUtil;
import com.jbpm.utils.JBPMUtil;
import com.kernal.annotation.WorkflowAction;
import com.kernal.annotation.WorkflowRejectCondition;
import com.kernal.utils.DateUtil;
import com.kernal.utils.ELUtil;
...
public class JbpmServiceImpl extends AbstractBaseServiceImpl  implements JbpmService 
{
	...
	*/
	...
	public  JSONArray  getWorkflowRejectBeanName(boolean isFilter,String sourceWorkflowName,String rejectWorkflowName) throws Exception
	{
			...
			Set<Class<?>> classes = FileUtil.getClasses("com.business.workflowRejectCondtion");
	...
	public  String  getWorkflowActions(String paramWorkflowName) throws Exception
	{
			...
			JSONArray jsonArray   = new JSONArray();
			Set<Class<?>> classes = FileUtil.getClasses("com.business.action");
			for(Class<?>  clazz :classes)
			{
	...
	/**
	 * (non-Javadoc)
	 * @see com.business.service.JbpmService#queryDeployedWorkflows()
	 **/
	@Override
	...
	public  synchronized String startDeployedProcess(HttpServletRequest request,String processDefinitionId,Map<String,String> modelData) throws Exception 
	{
			 ...
			 return this.workflowDataManage(request,null,null,null, deploymentImpl,processDefinition, processInstance,historyProcessInstanceImpl , null, activitiesDetailList, modelData, true,"start");
		}
	 /**
	 * (non-Javadoc)
	 * @see com.business.service.JbpmService#requestProcessTaskForm(javax.servlet.http.HttpServletRequest, java.lang.String, java.util.Map)
	 **/
	@Override
	...
	//获取提交列表信息
	@Override
	public Set<ActivityTaskUsers> getNextActivityTaskUsersList(HttpServletRequest request,String jhi_completedTaskImplId) throws Exception
	{
		...
		List<RouteInfo> routeInfos = JBPMUtil.getWorkFlowRouteInfos(jhi_completedTaskImpl,activityNames,modelData,this.jbpmDao.getHibernateTemplate(),this.jbpmDao.getJdbcTemplate());
	...
	public Set<ActivityTaskUsers> getBackActivityTaskUsersList(HttpServletRequest request,String jhi_completedTaskImplId) throws Exception
	{
		...
		List<RouteInfo>  allRouteInfos=JBPMUtil.getWorkFlowAllRouteInfos(deployId,/* activityDetail.getActivityName()*/activityDetail.getActivityKey());
		...
		String backworkflowname = request.getParameter("backworkflowname");
		if(null != backworkflowname && !"".equals(backworkflowname)){
			backworkflowname=URLDecoder.decode(backworkflowname, "UTF-8");
			for(int i_=0;i_<backRoutesArr.length;i_++)
			{
				String nodeNameKey = backRoutesArr[i_];
				if(nodeNameKey.equals(backworkflowname)){
	...
	//获取历史信息列表
	@Override
	public List<JBPMWorkflowHistoryInfo> getProcessedHistoryLogInfoList(String historyProcessInstanceImplDbid) throws Exception{
	...
	public void getRequestFormDealedInfo(HttpServletRequest request,JBPMWorkflowHistoryInfo jhi_completedTaskImpl,JBPMWorkflowHistoryInfo jhi_processInstance,Map<String,String> modelData,Set<String> activityNames) throws Exception
	{
		...
		modelData.put("currentTaskActivityName",/*historyTaskInstanceImpl.getActivityName()*/activityDetail.getActivityName());
	...
	public void addReadPersonToTask(JBPMWorkflowHistoryInfo jhi_completedTaskImpl,ActivityDetail assignToTaskAd,Map<String,String> modelData,String jbpmWorkflowHistoryInfoUserId,User readActor,String userIds,String splitStr,String readAdvise,List<User> userList) throws Exception
	{
	   ...
	   if(null != userList)
	   {
		   for(User u : userList)
		   {
			   ...
			   if(!isExist)
			   {
				   ...
				   //自动传阅发送消息提醒
				   this.sendMessageNoticeToUser(jhi_completedTaskImpl.getWorkflowDisplayName(), assignToTaskAd, readedActor, modelData, SubmitTypeEnum.READ,jhi_completedTaskImpl);
			   }
		   }
	   }
	   ...
	   for(String userId : userIdsList)
	   {
		   ...
		   if(!isExist)
		   {
			   ...
			   this.jbpmDao.getHibernateTemplate().save(readUser);
			   //继续传阅发送消息提醒
			   this.sendMessageNoticeToUser(jhi_completedTaskImpl.getWorkflowDisplayName(), jhi_completedTaskImpl.getActivityDetail(), readedActor, modelData, SubmitTypeEnum.READ,jhi_completedTaskImpl);
		   }
	   }
	}
	@Override
		   ...
		   if(null != userList)
		   {
			   for(User u:userList)
			   {
				   ...
				   if(!isExist)
				   {
					   ...
					   //会签发送消息提醒
					   this.sendMessageNoticeToUser(jhi_completedTaskImpl.getWorkflowDisplayName(), jhi_completedTaskImpl.getActivityDetail(), signaturedActor, modelData, SubmitTypeEnum.SIGNATURE,jhi_completedTaskImpl);
				   }
			   }
			   return;
		   }
	...
	@Override
	public synchronized String submitProcessedTaskForm(HttpServletRequest request,String currentTaskId, Map<String,String> modelData) throws Exception 
	{
...
//				Arrays.sort(nodesNameArr);
		...
		this.workflowDataManage(request,jbpmWorkflowHistoryInfoUserId,history_jhi_processInstance,jhi_completedTaskImpl_inner, deploymentImpl, processDefinition, processInstance,historyProcessInstanceImpl, completedTaskImpl, activitiesDetailList, modelData, false,flag);
		return saveActionReturnResult;
	}
	 /**
	 * (non-Javadoc)
	 * @see com.business.service.JbpmService#workflowDataManange(javax.servlet.http.HttpServletRequest, org.jbpm.pvm.internal.repository.DeploymentImpl, org.jbpm.api.ProcessDefinition, org.jbpm.api.ProcessInstance, org.jbpm.pvm.internal.task.TaskImpl, java.util.List, java.util.Map, boolean, java.lang.String)
	 **/
	@Override
							 ...
							 if(isNew)
							 {
								...
								// String actorValue = activityDetail.getActorValue();
								 ...
								 for(User u : actorList)
								 {
									 ...
									 this.sendMessageNoticeToUser(workflowDisplayName, pendingActivityDetail, u, modelData, SubmitTypeEnum.SUBMIT,jhi_task);
...
//									 }
	...
	@Override
	public void   sendMessageNoticeToUser(String workflowDisplayName,ActivityDetail pendingActivityDetail,User u,Map<String,String> modelData,SubmitTypeEnum ste,Object jbpmwork) throws Exception{
		//待办提醒
		 String messageTypes="";
		 
	     //if(!messageTypes.isEmpty()||!readmessageTypes.isEmpty()){
		 //JBPMWorkflowHistoryInfoUser jhi_task=(JBPMWorkflowHistoryInfoUser) jbpmwork;
		 //String ACTORID=jhi_task.getId();
	     String mailSubject  ="待办任务";
		 switch(ste){
		    case SUBMIT:{
		    	//messageContent = QueryUtil.getQueryString(modelData, StringUtil.nullToString(pendingActivityDetail.getMessageContent()));
		    	//messageContent+=workflowDisplayName;
		    	mailSubject  = "待办任务";
		        messageTypes   = StringUtil.nullToString(pendingActivityDetail.getMessageTypes());
		    	break;
		    }
		    case READ:{
		    	...
		    	//messageContent+=workflowDisplayName;
		    	mailSubject  = "传阅任务";
		    	messageTypes   = StringUtil.nullToString(pendingActivityDetail.getReadMessageTypes());;
		    	break;
		    }
		    case SIGNATURE:{
		    	...
		    	//messageContent+=workflowDisplayName;
		    	mailSubject  ="会签任务";
		    	messageTypes   = StringUtil.nullToString(pendingActivityDetail.getSignatureMessageTypes());;
		    	break;
		    }
		 }
		 if(!messageTypes.isEmpty()){
			 JBPMWorkflowHistoryInfo jbpmworkinfo=(JBPMWorkflowHistoryInfo) jbpmwork;
			 //String TASKID=jbpmworkinfo.getHistoryTaskInstanceImpl().getHistoryTask().getId();
			 //获取任务创建时间
			 //String show="[待办任务("+tasktime+")] ->"+jbpmworkinfo.getWorkflowName()+" ->"+jbpmworkinfo.getHistoryActivityName()+" ->"+jbpmworkinfo.getKeyTwo();
			 //String url="http://localhost:8080/deewinfl/jbpm/requestProcessTaskForm.action?currentTaskId="+TASKID+"&jbpmWorkflowHistoryInfoUserId="+ACTORID;
			 
			 String tasktime=DateUtil.getDateTime(jbpmworkinfo.getHistoryTaskInstanceImpl().getHistoryTask().getCreateTime());
			 String url=ResourceUtil.getConfigValue("sso.localClient");
			 String show="["+mailSubject+"("+tasktime+")] ->"+jbpmworkinfo.getWorkflowName()+" ->"+jbpmworkinfo.getHistoryActivityName()+" ->"+jbpmworkinfo.getKeyTwo();
			 String messageContent="<a target='_blank' href='"+url+"'>"+show+"</a>";
			 
			 String mailSubjecttitle  =workflowDisplayName+mailSubject;
			 for(String messageType :messageTypes.split(",")){
				 switch(MessageTypeEnum.valueOf(messageType.toUpperCase())){
				   case SMS:{
					   String receiverMobilePhone    = u.getTelephone();
				   ...
				   case MAIL:{
					   String mailToAddress = u.getEmail();
					   SendEmailUtil.sendSimpleMessage(mailToAddress, mailSubjecttitle, messageContent);
					   break;
				   }
				   case STATIONMESSAGE:{
						//发站内信给经销商，提报GPS信息
	...
	public String removeProcessInstance(HttpServletRequest request,String processInstanceDBID) throws Exception
	{
		...
		*****/
		...
		Assert.assertEquals(1, deploymentImpls.size());
...
//		}
	...
	@Override
	public List<JBPMWorkflowHistoryInfo> getDealerSummary(String keyFive)
			throws Exception {
		String queryProjSummaryHsqlHistoryTaskInstanceImplIds = "select max(jwhi.historyTaskInstanceImpl.dbid) from JBPMWorkflowHistoryInfo jwhi where jwhi.historyTaskInstanceImpl is not null and jwhi.keyFive = ? group by jwhi.deploymentImpl,jwhi.historyProcessInstanceImpl";
	    List<Long> historyId_list  = this.findResultsByHSQL(queryProjSummaryHsqlHistoryTaskInstanceImplIds, keyFive) ;   
        Long[] historyTaskInstanceIds = new Long[historyId_list.size()];
        historyTaskInstanceIds = historyId_list.toArray(historyTaskInstanceIds);
        String queryProjSummaryHsql = "from JBPMWorkflowHistoryInfo jwhi where  jwhi.historyTaskInstanceImpl is not null and  jwhi.historyTaskInstanceImpl.dbid in(:historyTaskInstanceIds) order by jwhi.historyTaskInstanceImpl.startTime";
        List<JBPMWorkflowHistoryInfo> summayHistoryInfos = this.jbpmDao.findResultsByNamedParamHSQL(queryProjSummaryHsql, new String[]{"historyTaskInstanceIds"}, new Object[]{historyTaskInstanceIds});
