 ...
 * 
 */
@WorkflowAction(name = "dealerFundCollectionEditAction", description = "流程编辑动作", workflowName = "经销商资金收款")
@Component(value = "dealerFundCollectionEditAction")
public class DealerFundCollectionEditAction implements JbpmBaseAction {
	...
	public void end(HttpServletRequest request, Map<String, String> variablesMap, JBPMWorkflowHistoryInfo jbpmWorkflowHistoryInfo) throws Exception {
		...
		for (int i = 0; i < jsonFundString_Array.length(); i++) {
			...
			//增加对资金计划的判断
			ContractFundFundPlan fplan=this.tableService.findEntityByID(ContractFundFundPlan.class, jsonFundString_object.get("fundfundchargeplan").toString());
			if(fplan==null)
			{
