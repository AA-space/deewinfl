...
function doCheckFundCharge(fundData,tabledata){
	...
	for(var i=0;i<tabledata.length;i++){
       for(var j=0;j<fundList.length;j++){
           if(fundList[j].paymentid==tabledata[i].paymentid&&fundList[j].feetype==tabledata[i].feetype&&fundList[j].paytype==tabledata[i].paytype){//查找相同的付款编号
             ...
             if(parseFloat(temp)>0){
                 //检查是否大于计划
                  if(parseFloat(fundList[j].lastmoney)<parseFloat(temp)){
                	  messageovpaln.push(tabledata[i].rawValue_feetype+"第"+fundList[j].paymentid+"笔");
                  }
                  if(ebankId==tabledata[i].ebanknumber)
                   {
                	  ebankMoney=parseFloat(ebankMoney)-parseFloat(tabledata[i].factmoney);
                  ...
                  if(ebankMoney<0){
                	  messageovebank.push(tabledata[i].rawValue_feetype+"第"+fundList[j].paymentid+"笔");
                  }
             }else{
	...
	if(messageovpaln.length>0||messageovebank.length>0||messageovreback.length>0){
		var message="";
		if(messageovpaln.length>0){message=message+messageovpaln+"收款金额大于计划";}
		
		if(messageovebank.length>0){if(message!=""){message=message+"<br>";} message=message+""+messageovebank+"的金额大于网银金额";}
...
function checkFundCharge(){
	 ...
	 var tabledata=util.clone(currentTable1.getRowsData());
...
jQuery(function(){
	    ...
	    columns:[
				 ...
				 {name:'rawValue_feetype',  header:'费用类型',nullable:false,type:'combobox',hiddenName:'feetype',
			       ...
			       {name:'rawValue_settlemethod',  header:'结算方式',readOnly:true,nullable:false,type:'combobox',hiddenName:'settlemethod',
			    	   config:{
					       width:165,
					       xmlFileName:'\\combos\\comboDict.xml',
					       loadMode:'ajax',
			       ...
			       {name:'feeadjust',  header:'调整金额',nullable:false,hidden:true,type:'double',otherAttributes:'id="re_feeadjust" onkeyup="comptuerPartFeeMoney()"'},
			       ...
			       {name:'factobject',  header:'付款客户',nullable:false,type:'combobox',readOnly:true,
					    config:{
						       width:165,
						       xmlFileName:'\\eleasing\\workflow\\ebank\\comboEbankSelect.xml',
						       loadMode:'ajax',
			       ...
			       {name:'clientaccnumber',  header:'客户帐号',nullable:false,type:'combobox',
					    config:{
						       width:165,
						       xmlFileName:'\\eleasing\\workflow\\ebank\\comboEbankSelect.xml',
						       loadMode:'ajax',
			       ...
			       {name:'paytypename',header:'收付类型',hidden:true,defaultValue:"收款"},
