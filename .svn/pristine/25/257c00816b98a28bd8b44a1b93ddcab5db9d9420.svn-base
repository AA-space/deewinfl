...
@Service(value = "taxVatUploadInfoService")
public class TaxVatUploadInfoServiceImpl implements TaxVatUploadInfoService {
	...
	public void createVoucherTaxVat(ContractInfo cinfo,BigDecimal sum) throws Exception {
		...
		String workFlowName = "车款-进项发票";                                                      //流程名称
		BigDecimal originalAmount = sum;                                                                                           //原币金额
		String voucherAbstract = "收到"+cinfo.getCustId().getCustName()+"发票";                                  //摘要
		/** 本凭证-公用字段-结束*/
		...
		VoucherassStactsConfig config_01 = this.baseService.findEntityByID(VoucherassStactsConfig.class, "239");   //凭证配置表:应交税费-应交增值税-进项税额
		...
		/** 其他应收款-本金进项税额*/
		/************************************** 凭证体2-分割线-开始 ********************************************************/
		VoucherassStactsConfig config_02 = this.baseService.findEntityByID(VoucherassStactsConfig.class,"217");        //凭证配置表:其他应收款-本金进项税额
	...
	public void adjustFactorageCharge(TaxFundUploadInfo taxFundUploadInfo,BigDecimal originalAmount) throws Exception {
		...
		String workFlowName = "调整税额尾差-手续费"; //流程名称
		ContractInfo contractInfo = this.baseService.findEntityByID(ContractInfo.class, taxFundUploadInfo.getContractId());
		...
		VoucherassStactsConfig config_01 = this.baseService.findEntityByID(VoucherassStactsConfig.class, "233");   //凭证配置表:主营业务收入-手续费收入
		String voucherAbstract_01 = "主营业务收入-手续费收入-‘" + custInfo.getCustName()+"‘";//摘要
		...
		body_01.setAccountNumber(config_01);                 //这个科目
		body_01.setOriginalAmount(originalAmount);      //原币金额 
		body_01.setEntryDC(CR);                            //借贷方向 （DR(1) 借方- CR(1) 贷方）
		...
		/** 其他应收款-本金进项税额*/
		/************************************** 凭证体2-分割线-开始 ********************************************************/
		VoucherassStactsConfig config_02 = this.baseService.findEntityByID(VoucherassStactsConfig.class,"238");        //凭证配置表:应交税费-应交增值税-销项税额
		String voucherAbstract_02 = "应交税费-应交增值税-" +new BigDecimal("-" + originalAmount.toString());//摘要
		...
		//构建凭证体1需要的参数:
		body_02.setAccountNumber(config_02);                       //这个科目对象
		body_02.setOriginalAmount(new BigDecimal("-" + originalAmount.toString()));               //原币金额 
		body_02.setEntryDC(CR);                                    //借贷方向 （DR(1) 借方- CR(1) 贷方）
	...
	public void adjustAmtCharge(TaxVatUploadInfo taxVat, BigDecimal originalAmount) throws Exception{
		...
		String workFlowName = "调整税额尾差-车款"; //流程名称
		ContractInfo contractInfo = this.baseService.findEntityByID(ContractInfo.class, taxVat.getContractId());
		
		CustInfo custInfo = contractInfo.getCustId();
		String voucherAbstract = "调整客户车款税额尾差";//摘要
		...
		VoucherassStactsConfig config_01 = this.baseService.findEntityByID(VoucherassStactsConfig.class, "219");   //凭证配置表:主营业务收入-手续费收入
		...
		/************************************** 凭证体2-分割线-开始 ********************************************************/
		VoucherassStactsConfig config_02 = this.baseService.findEntityByID(VoucherassStactsConfig.class,"217");        //凭证配置表:应交税费-应交增值税-销项税额
		...
		/************************************** 凭证体3-分割线-开始 ********************************************************/
		VoucherassStactsConfig config_03 = this.baseService.findEntityByID(VoucherassStactsConfig.class,"219");        //凭证配置表:应交税费-应交增值税-销项税额
		...
		/************************************** 凭证体4-分割线-开始 ********************************************************/
		VoucherassStactsConfig config_04 = this.baseService.findEntityByID(VoucherassStactsConfig.class,"219");        //凭证配置表:应交税费-应交增值税-销项税额
		...
		body_04.setOriginalAmount(new BigDecimal("-" + originalAmount.toString()));                 //原币金额 
	...
	public void adjustRent(TaxRentUploadInfo taxRui, BigDecimal originalAmount) throws Exception {
		...
		String workFlowName = "调整税额尾差-租金"; //流程名称
		ContractInfo contractInfo = this.baseService.findEntityByID(ContractInfo.class, taxRui.getContractId());
		...
		VoucherassStactsConfig config_01 = this.baseService.findEntityByID(VoucherassStactsConfig.class, "232");   //凭证配置表:主营业务收入-手续费收入
		String voucherAbstract_01 = "主营业务收入-租金利息收入-‘" + custInfo.getCustName()+"‘";//摘要
		...
		/************************************** 凭证体2-分割线-开始 ********************************************************/
		VoucherassStactsConfig config_02 = this.baseService.findEntityByID(VoucherassStactsConfig.class,"238");        //凭证配置表:应交税费-应交增值税-销项税额
		String voucherAbstract_02 = "应交税费-应交增值税-销项税额";//摘要
