...
public class FundIncomeServiceImpl implements FundIncomeService {
	...
	public void createVoucherForPenalty(ContractInfo cinfo,JSONObject jsonObj,String workname) throws Exception {
		...
		/** 本凭证-公用字段-结束*/
		//不含税金额
		BigDecimal sum =new BigDecimal(jsonObj.optString("penalty")).setScale(2, BigDecimal.ROUND_HALF_EVEN);
		...
		VoucherassStactsConfig config_01 = this.baseService.findEntityByID(VoucherassStactsConfig.class, "225");                          //凭证配置表
		String voucherAbstract_01 = "确认"+cinfo.getCustId().getCustName()+"第"+planlist+"期延期利息";//摘要
	...
	public void createVoucherForPenaltyhx(ContractInfo cinfo,JSONObject jsonObj,FundEbankData fbd) throws Exception {
		...
		Integer DR = 1;                                                                                                                                 //借
		Integer CR = -1;                                                                                                                                //贷
		BigDecimal sum=new BigDecimal(jsonObj.optString("penalty"));
		String planlist=jsonObj.optString("planlist");
		/** 公用字段-结束*/
		...
		String voucherAbstract_01 = "核销"+cinfo.getCustDealer().getCustName()+"-"+cinfo.getContractNumber()+"-"+cinfo.getCustId().getCustName()+"第"+planlist+"期延期利息";//摘要
		if(fbd!=null){
			voucherAbstract_01 = fbd.getSn()+"核销"+cinfo.getCustDealer().getCustName()+"-"+cinfo.getContractNumber()+"-"+cinfo.getCustId().getCustName()+"第"+planlist+"期延期利息";//摘要
