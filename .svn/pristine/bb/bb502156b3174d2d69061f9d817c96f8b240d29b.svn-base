...
<input type="hidden" id="id_contract_info_repealdate" 	name="contract_info.repealdate" value="${requestScope['contract_info.repealdate']}"></input>
<input type="hidden" id="id_contract_info_repealreason" name="contract_info.repealreason" value="${requestScope['contract_info.repealreason']}"></input>
<input type="hidden" id="id_contract_info_dictionarydatabyrepealreasontype" name="contract_info.dictionarydatabyrepealreasontype" value="${requestScope['contract_info.dictionarydatabyrepealreasontype']}"></input>
<input type="hidden" id="id_rawValue_contract_info_dictionarydatabyrepealreasontype" name="rawvalue_contract_info.dictionarydatabyrepealreasontype" value="${requestScope['rawvalue_contract_info.dictionarydatabyrepealreasontype']}"></input>
...
<div id="contract_approval_form">
	...
	<div class="mini-panel" title="项目商务条件" maskOnLoad="false" showCollapseButton="true" style="width: 100%">
			<div class="mini-tabs"  plain="false" style="width: 100%;height:auto" id="tab1">
				<div title="合同撤销信息" name="changeList" iconCls="icon-cut" url="<%=request.getContextPath()%>/workflow/forms/contract_nocar/contract_cancel/contract_cancel_info.bi?isViewHistoryTask=true">
				</div>
...
function fillHiddenVal(type){
	...
	if(typeof(iframe0)!='undefined'){
		...
		//保存子页面数据
		var o = grid0.getData();
		var repealdate=o.contract_info.repealdate;
		$("#id_contract_info_repealdate").val(mini.formatDate (repealdate,"yyyy-MM-dd"));
		
		var repealreason=o.contract_info.repealreason;
		$("#id_contract_info_repealreason").val(repealreason);
		$("#id_contract_info_dictionarydatabyrepealreasontype").val(o.contract_info.dictionarydatabyrepealreasontype);
		$("#id_rawValue_contract_info_dictionarydatabyrepealreasontype").val(o.rawValue_contract_info.dictionarydatabyrepealreasontype);
		if("submit"==type){
		   grid0.validate();
