...
//是否提交    
	...
	{
		 fillHiddenVal();
		 if (submitButtonDisplayText == buttonText) //提交时候验证
		 {
		    var submitProcessedForm = document.getElementById("id_submitProcessedForm");
			var gpsValue = getTracywindyObject('id_combo_is_gps').getValue();
			if ("" == gpsValue || 1 == gpsValue || "1" == gpsValue) 
			{
				alert("GPS信息未确认，无法提交下一步。");
				return false;
			}
		  } 
		  if (backButtonDisplayText == buttonText) //退回时候验证
		  {
			    return true;
		  }
		  return true;
...
<div class="fillTableContainer">
   ...
   <tr id="fund_money_collectioninfo">
			<td colspan=4>
				<!-- 未付设备款明细 --> 
				<jsp:include page="gps_info_two.jsp"></jsp:include></td>
		</tr>
...
<jsp:include page="gps_info_manage2.jsp"></jsp:include>
