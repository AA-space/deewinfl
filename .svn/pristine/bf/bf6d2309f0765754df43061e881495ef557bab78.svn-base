...
jQuery(function() {
	...
	function formPageReadOnlyCallBack(){
		formJSPInputHideOrShow("id_contract_doc_ems_window","",1);
	}
...
<input type="hidden" id="id_contract_doc_send_info_str2" name="json_contract_doc_send_info_str" value='${empty json_contract_doc_send_info_str ? "[]" : json_contract_doc_send_info_str }'></input>
   ...
   <table class="fillTable" cellspacing="0" cellpadding="0">
	<tr class="tr-contract-business-condition">
		...
		<div id='id_contract_info_tabs_content' style="">
			<div title="合同登记">
				<span class="print-tabs-title-content">合同登记</span>
				<script type="text/javascript">
					    jQuery(function() {
						    isViewHistoryTask = false;
						    });
		            </script>
				<div id="id_table_leasing_contract_doc_send_info"	style="overflow: hidden;">
					<jsp:include page="contract_doc_send_info.jsp"></jsp:include>
					<script type="text/javascript">
					    jQuery(function() {
...
<jsp:include page="contract_doc_ems_info.jsp"></jsp:include><%--前提条件 --%>
<script>

function fillHiddenVal() {
	var contractEquip = getTracywindyObject('table_id_table_leasing_contract_doc_send_info');//合同登记
	var s=jQuery('#id_contract_doc_send_info_str2').val(
			JsonUtil.encode(contractEquip.getRowsJsonData()));
	//console.debug(JsonUtil.encode(contractEquip.getRowsJsonData()));
}
</script>