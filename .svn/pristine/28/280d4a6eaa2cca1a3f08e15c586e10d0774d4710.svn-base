...
jQuery(function(){
	//dictYesOrNoByCode('id_isAffiliated','isAffiliated',1,'','','require="true" label="是否挂靠"');
	    	 ...
	    	 if(operType=="update"){
				  for(var i=0;i<me.comboboxs.length;i++){
				       ...
				       if(combo.otherConfig.name=="modelname"){
					    	   ...
					    	   combo.reload();*/
					    	   var equipclass=rowData["equipclass"];
					    	   var modelname=rowData["modelname"];
							   var loadCompleteFunc = (function(combo,equipclass,modelname){
							   	   return function($combo,rowData){
									   combo.setParams({equipclass:equipclass});
							    	   combo.value = modelname;
							    	   combo.reload();
									   $combo.loadComplete = null;
								   };
							   })(combo,equipclass,modelname);
		...
		/*    {name:'equiptypename', header:'车辆类别',type:'combobox',readOnly:true,nullable:false,hiddenName:'equiptype',
		   ...
		   {name:'chassisnum',  header:'车架号',otherAttributes:'maxB="100"',otherAttributes:'id="id_chassisnum"',nullable:false},
