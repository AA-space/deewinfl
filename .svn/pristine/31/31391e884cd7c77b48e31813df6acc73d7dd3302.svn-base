...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        params: [],                         //提交到服务器的参数 //edit by jason 2012-5-29 统一名称
        columns: [],                          //数据源
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        removeParm : function(name)
        {
            ...
            {
                removeArrItem(params, function (p) { return p.name == name; }); 
            } else
            {
                delete params[name];
            }
        ...
        //edit by jason 2013-5-29 异步加载展开的数据
		loadComponentData : function(rowParm) {
			var g = this, p = this.options;
			var columnParams = [];
			var rowdata = g.getRow(rowParm);
			if (p.columnParams) {

				if ( typeof p.columnParams == 'function') {
					columnParams = p.columnParams(rowParm);
				}
			}
			jQuery.post(p.url, columnParams, function(v) {
				g.trigger('ChildSuccess', [v, rowdata, g]);
				rowdata[p.tree.childrenName] = v;

				var indexInCollapsedRows = $.inArray(rowdata.__recordid, g.collapsedRows);
				g.collapsedRows.splice(indexInCollapsedRows, 1);
				g._showData();
				rowdata.__isLoadData = true;
			});
		},
        loadServerData: function (param, clause)
        {
            ...
            var ajaxOptions = {
                ...
                success: function (data)
                {
                    ...
                    //edit by jason 2013-5-29 如果返回数据没有为集合类型，没有 p.root则加一个，增加容错
        ...
        */
        ...
        _addNewRecord: function (o, previd, pid)
        {
            ...
            o['__previd'] = previd;
            //edit by jason 2013-5-29 忘了
			g.recordIndexNumber = g.recordIndexNumber || 1;
			if (!o['__recordid']) {
				o['__recordid'] = o['__id'] + '_' + g.recordIndexNumber;
				g.recordIndexNumber++;
			}
            if (previd && previd != -1)
            {
        ...
        getColumn: function (columnParm)
        {
            ...
            if (typeof columnParm == "string") // column id
            {
				...
				else if (parseInt(columnParm) || parseInt(columnParm) == 0) {//edit by jason 2013-5-29 增加容错
					return g.columns[parseInt(columnParm)];
				} else {
					for (var key = 0; key < g.columns.length; key++) {
						if (g.columns[key]['columnname'] === columnParm) {
							return g.columns[key];
						}
					}
				}
			} else if ( typeof (columnParm) == "number")// column
            {
        ...
        //edit by jason 2013-5-29 通过recordid获取记录
		getRecord : function(recordId) {
			var g = this, p = this.options;
			for (var key in g.records) {
				if (g.records[key].recordId == recordId) {
        ...
        //edit by jason 2013-5-29  根据标签判断是否展开
		_openChild : function(o, g) {
			var currentChildren = g.getChildren(o, false);
			for (var j = 0; j < currentChildren.length; j++) {
				var oc = currentChildren[j];
				var currentChildrenRow = $([g.getRowObj(oc['__id'])]);
				if (g.enabledFrozen())
					currentChildrenRow = currentRow.add(g.getRowObj(oc['__id'], true));

				currentChildrenRow.show();
				if ($(".l-grid-tree-link-open", currentChildrenRow).length > 0) {
		...
		toggle : function(rowParm) {
			...
			var g = this, p = this.options;
			var rowdata = g.getRow(rowParm);
			//edit by jason 2013-5-29  判断tree数据是否已经加载，打开tree子项
			if (!rowdata.__isLoadData) {
				g.loadComponentData(rowParm);
			} else {
				g._toggleData(g, p, rowdata);
			}

		},

		_toggleData : function(g, p, rowdata) {
			...
			if (linkbtn.hasClass("l-grid-tree-link-close"))// 收缩
			{
				...
				indexInCollapsedRows = $.inArray(rowdata.__recordid, g.collapsedRows);
			...
			} else// 折叠
			{
				...
				indexInCollapsedRows = $.inArray(rowdata.__recordid, g.collapsedRows);
				if (indexInCollapsedRows == -1)
					g._pushCollapsedRows(rowdata);
			}
			var children = g.getChildren(rowdata, false);
			for (var i = 0, l = children.length; i < l; i++) {
				var o = children[i];
				...
				if (opening) {
					currentRow.show();
					if ($(".l-grid-tree-link-open", currentRow).length > 0) {
						if (o.__hasChildren) {
							g._openChild(o, g);
						}
					}
					...
					if (o.__hasChildren) {
						var currentChildren = g.getChildren(o, true);
						for (var j = 0; j < currentChildren.length; j++) {
							var oc = currentChildren[j];
							var currentChildrenRow = $([g.getRowObj(oc['__id'])]);
							if (g.enabledFrozen())
								currentChildrenRow = currentRow.add(g.getRowObj(oc['__id'], true));
							currentChildrenRow.hide();
						}
					}
					currentRow.hide();
				}
        ...
        _showData: function ()
        {
            ...
            g.expandedRows = g._getExpandedRows();
            if (p.usePager)
            {
                //更新总记录数
        ...
        _getTreeCellHtml: function (oldContent, rowdata)
        {
            ...
            var g = this, p = this.options;
			var _i = p.tree.isExtend(rowdata, p);
			g.collapsedRows = g.collapsedRows || [];
			var rid = rowdata.__recordid;
			if (!_i && $.inArray(rowdata.__recordid, g.expandedRows) == -1)
				g._pushCollapsedRows(rowdata);
			var isExtend = $.inArray(rowdata.__recordid, g.collapsedRows || []) == -1;
		...
		_pushCollapsedRows : function(record) {
			var g = this;
			if (record.__hasChildren && $.inArray(record.__recordid, g.collapsedRows) == -1) {
				g.collapsedRows.push(record.__recordid);
			}
		},
		...
		_getExpandedRows : function() {
			var g = this, p = this.options;
			var expendRecords = [];
			if (g.records && g.collapsedRows) {
				for (var key in g.records) {
					if (g.records[key]['__hasChildren'])
						expendRecords.push(g.records[key]['__recordid']);

				}
				for (var i in g.collapsedRows) {
					var s = g.collapsedRows[i];
					var _index = $.inArray(g.collapsedRows[i], expendRecords);
					if (_index != -1)
						expendRecords.splice(_index, 1);
				}
    	...
    	destroy : function() {
			var g = this, p = this.options;
			$(document).unbind("mouseup." + g.id);
			$(document).unbind("click." + g.id);
			$(window).unbind("resize." + g.id);
			$(document).unbind("keydown." + g.id);
			$(document).unbind("keyup." + g.id);
