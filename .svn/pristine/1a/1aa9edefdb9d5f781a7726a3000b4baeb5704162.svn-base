...
isViewHistoryTask = true;
	...
	function workflowSubmitCallBack()//返回true提交，返回false不提交 ，该回调函数可与用表单字段验证
	{
		...
		//return isPassed;
		if(!checkTracywindyTableData("table_id_table_leasing_contract_fund_payment_now_container","本次付款明细")){return false;}//本次付款明细
		return true;
	}
		...
		<div id="id_table_leasing_contract_fund_payment_now_container"
			...
			<script language="javascript">
               jQuery(function() {
				...
				<jsp:include page="fund_payment_now06.jsp"/>
				 <script language="javascript">
			       jQuery(function() {
				       isViewHistoryTask = true;
				       });
