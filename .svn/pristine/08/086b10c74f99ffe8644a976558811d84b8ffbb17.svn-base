<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	jQuery(function(){
	 //加投票按钮       
	 if(!isCompletedTask){
	  jQuery("#id_btn_tools_container").append("<a id='' class='easyui-linkbutton l-btn l-btn-plain' plain='true' onclick='addMenuData();' href='javascript:void(0);'><span class='l-btn-left'><span class='l-btn-text icon-ok' style='padding-left: 20px;'>投票</span></span></a>");
	 }
    });	
	function checkVote(){
		loadMask_oper.show();
		var isSubmit = false;
		 ...
		 ajaxRequest({
  		     ...
  		     success:function(res){
  	   		    ...
  	   		    loadMask_oper.hide();
  	   		    if(svote==""){
	  	   		    jQuery.messager.alert('错误提示',"<div style='font-size:18px;line-height:30px;'>请您投票！</div>",'error');
	  	   		    isSubmit = false;
  	   		    }else{
  	   		        isSubmit = true;
  	   	   		}
...
<table>
	<tr>
		<td class="input_label_desc" style="width:30%"> <input id="id_proj_vote" name="proj_vote.id" class="td-content-input td-content-readonly" readOnly  type="hidden"  value=""/>表决意见:</td>
		<td class="input_value"><div class="leftComboContainer" id="id_vote_type"> </div><span class="content-required">*</span></td>
    </tr>
    <tr style="display:none">
		<td class="input_label_desc">评分:</td>
		<td class="input_value"><input type="text"  id="id_votegrade"  name="proj_vote.votegrade" value="0.00" style="width:30" />
