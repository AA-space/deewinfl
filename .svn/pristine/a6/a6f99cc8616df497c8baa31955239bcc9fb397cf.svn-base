        ...
        <table class="miniext-form-fit">
            <tr>
	            <td>
	                ...
	                <a class="mini-button" iconCls="icon-addfolder" plain="true" onclick="importRow">导入租赁物</a>
	            </td>
            </tr>
        </table>
    </div>
...
var importcolumn= [
                     ...
                     {header:'设备名称',mapping:'eqipname'},
                     {header:'规格/型号',mapping:'model'},
	  		         {header:'数量',mapping:'equipnum'},
		             {header:'单价',mapping:'price'},
		             {header:'总价',mapping:'total'},
		             {header:'品牌',mapping:'brand'},
		             {header:'供应商',mapping:'vndr', fieldclass:'CustInfo',field:'custName'},
		             {header:'制造商',mapping:'manufacturer', fieldclass:'CustInfo',field:'custName'},
		             {header:'设备类型',mapping:'devicetype',fieldclass:'DictionaryData',field:'name'},
		             {header:'机器序列号',mapping:'eqipid'},
		  	         {header:'交付地点',  mapping:'equipdeliveryplace'},
		  	         {header:'交付时间',  mapping:'equipdeliverydate'},
		  	         {header:'设置地点',  mapping:'equipplace'},
		  	         {header:'备注', mapping:'cenote'}
	        ];
	        
function importRow() {
    
    attachmentUp3({url:"/leasing/template/uploadExcelToDatabase.action",checkColumnEmpty:"1",isSaveTo:"savepage",targetClass:'com.business.entity.proj.equip.ProjEquip',modelname:'equipinfo',title:'租赁物件上传',parames:importcolumn});
    
}

function importEbankCallBack(message){
  var projequip=eval("("+message+")");
  if(projequip.result=="true"){
      alert(projequip.message);
      mini.get("id_downOrLoadFormWindow").hide();
      grid.addRows(projequip.fileupdatedate);
