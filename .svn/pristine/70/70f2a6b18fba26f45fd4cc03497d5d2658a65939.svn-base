/**
 * shiftzoom.js 3.2 (16-Sep-2009) (c) by Christian Effenberger All Rights Reserved. Source: shiftzoom.netzgesta.de
 * Distributed under Netzgestade Software License Agreement. This license permits free of charge use on non-commercial
 * and private web sites only under special conditions. Read more at... http://www.netzgesta.de/cvi/LICENSE.txt
 * 
 * syntax: shiftzoom.defaultFading = true; //BOOLEAN startup fading shiftzoom.defaultButtons = true; //BOOLEAN left top
 * info & zoom buttons shiftzoom.defaultNozoom = false; //BOOLEAN disable zooming feature shiftzoom.defaultBicubic =
 * false; //BOOLEAN enable MS bicubic image interpolation mode for IE7+ shiftzoom.defaultZoom = 0; //INT/FLOAT 0-100 (%)
 * zooming percentage shiftzoom.defaultXpos = 50; //INT/FLOAT 0-100 (%) horizontal position shiftzoom.defaultYpos = 50;
 * //INT/FLOAT 0-100 (%) vertical position shiftzoom.defaultOpacity = 90; //INT 0-100 (%) zoom button opacity
 * shiftzoom.defaultMillisec = 40; //INT 5-100 zoom interval delay shiftzoom.defaultIntitle = ''; //STR 'click or press
 * shift key to zoom in' shiftzoom.defaultOuttitle = ''; //STR 'click or press alt key to zoom out'
 * shiftzoom.defaultInfoblock = ''; //STR '
 * <dl>
 * <div align="right">Mouseover <big>Keyboard Support<\/big><\/div>
 * <dt>Zoom in:<\/dt>
 * <dd>[+] / [PgDn] / [End]
 * <em>(is faster)<\/em> /<br \/> [shift] + <u>left</u> mouse button /<br \/><u>middle</u> / <u>wheel</u> mouse button /<br \/>mouse wheel <u>down</u><small><br \/><br \/><\/small><\/dd><dt>Zoom out:<\/dt><dd>[–] / [PgUp] / [Home] <em>(is faster)<\/em> /<br \/> [alt] + <u>left</u> mouse button /<br \/><u>right</u> mouse button /<br \/>mouse wheel <u>up</u><small><br \/><br \/><\/small><\/dd><dt>Pan / Shift / Move:<\/dt><dd>[left] / [right] / [down] / [up] arrow buttons<br \/>+ [shift] <em>(is faster)</em> and + [alt] <em>(is slower)<\/em><\/dd><\/dl>'
...
var cvi_sztimer, cvi_szactive, cvi_szimage = null, shiftzoom = {
	...
	add : function(ele, opts) {
		...
		if (!ele.active) {
			...
			if (document.images && document.createElement && document.getElementById && document.getElementsByTagName) {
				var st, over, view, div = ele.parentNode, img = shiftzoom.E('div'), xref = shiftzoom.E('img'), outer = shiftzoom.E('div');
				...
				if (ele.width >= 100 && ele.width < img.maxwidth && ele.height >= 100 && ele.height < img.maxheight) {
					...
					img.webkit = window.atob != undefined && !window.updateCommands ? 1 : 0;
					img.divbug = !img.webkit && navigator.userAgent.indexOf('WebKit') > -1 ? 1 : 0;
					img.gecko = navigator.userAgent.indexOf('Gecko') > -1 && window.updateCommands ? 1 : 0;
					...
					if (img.showcoords) {
						img.onmousemove = (img.pixelcoords ? shiftzoom._showCoords : img.percentcoords ? shiftzoom._showPercent : shiftzoom._showLatLon);
					}
		...
		if (img && typeof (v) === "object" && typeof (img.ctrlid) === "string") {
			...
			for (i = 0; i < v.length; i++) {
				...
				if (!ele && w >= 8 && h >= 8 && s != '') {
					...
					ie.style.height = q ? h + 'px' : (h / (img.maxheight / 100)) + '%';
					n = (img.maxheight / 100);
					m = (img.maxwidth / 100);
					if (q) {
		...
		if (img && typeof (img.ctrlid) === "string") {
			if (d && typeof (v) === "boolean") {
				...
				case 'showcoords':
					...
					break;
				case 'pixelcoords':
					if (v == false && img.pixelcoords == true && img.showcoords == true) {
						img.onmousemove = shiftzoom._showLatLon;
					} else if (v == true && img.pixelcoords == false && img.showcoords == true) {
						img.onmousemove = shiftzoom._showCoords;
					}
					img.pixelcoords = v;
					break;
				case 'percentcoords':
					if (v == false && img.percentcoords == true && img.showcoords == true) {
						img.onmousemove = shiftzoom._showLatLon;
					} else if (v == true && img.percentcoords == false && img.showcoords == true) {
				...
				case 'buttons':
					...
					break;
				case 'zoomin':
					if (!img.nozoom && !img.buttons && (parseInt(img.parentNode.style.width) < img.maxwidth || parseInt(img.parentNode.style.height) < img.maxheight)) {
						if (v == true) {
							img.zoomin = true;
							img.zoomout = false;
							if (img.gecko) {
					...
					break;
				case 'zoomout':
					if (!img.nozoom && !img.buttons && (parseInt(img.parentNode.style.width) > img.minwidth || parseInt(img.parentNode.style.height) > img.minheight)) {
						if (v == true) {
							img.zoomout = true;
							img.zoomin = false;
							if (img.gecko) {
	...
	_showCoords : function(e) {
		if (cvi_szactive != null) {
			var k, t, x, y, ex, ey, px = 0, py = 0, o = shiftzoom.G(cvi_szactive), w = o.parentNode.width, h = o.parentNode.height;
			e = e ? e : window.event;
			ex = e.clientX;
			ey = e.clientY;
			if (e.pageX || e.pageY) {
	...
	_showPercent : function(e) {
		if (cvi_szactive != null) {
			var k, t, x, y, z, ex, ey, px = 0, py = 0, na = !Number.prototype.toFixed ? 0 : 1, o = shiftzoom.G(cvi_szactive), w = o.parentNode.width, h = o.parentNode.height;
			e = e ? e : window.event;
			ex = e.clientX;
			ey = e.clientY;
			if (e.pageX || e.pageY) {
	...
	_zoomIn : function(id, ct, st, sw, ew, sh, eh, sx, ex, sy, ey, nz) {
		if (!nz) {
			...
			if (parseInt(obj.parentNode.style.width) < obj.maxwidth || parseInt(obj.parentNode.style.height) < obj.maxheight) {
				...
				cvi_sztimer = setInterval("shiftzoom._zoomIn('" + id + "'," + ct + "," + st + "," + sw + "," + ew + "," + sh + "," + eh + "," + sx + "," + ex + "," + sy + "," + ey + "," + nz + ")",
						obj.millisec);
			} else {
				clearInterval(cvi_sztimer);
			}
	...
	_zoomOut : function(id, rm, ct, st, sw, ew, sh, eh, sx, ex, sy, ey, nz) {
		if (!nz) {
			var mw, mh, mx, my, obj = shiftzoom.G(id);
			if (parseInt(obj.parentNode.style.width) > obj.minwidth || parseInt(obj.parentNode.style.height) > obj.minheight) {
				clearInterval(cvi_sztimer);
				mw = Math.max(obj.minwidth, Math.min(obj.maxwidth, Math.round(ew * ct / st + sw)));
				mx = Math.round(ex * ct / st + sx);
				mh = Math.max(obj.minheight, Math.min(obj.maxheight, Math.round(eh * ct / st + sh)));
				...
				shiftzoom.G(obj.textid).innerHTML = parseInt((mw / obj.minwidth) * 100) + " / " + parseInt(obj.xfactor * 100) + " %";
				cvi_sztimer = setInterval("shiftzoom._zoomOut('" + id + "'," + rm + "," + ct + "," + st + "," + sw + "," + ew + "," + sh + "," + eh + "," + sx + "," + ex + "," + sy + "," + ey + ","
						+ nz + ")", obj.millisec);
			} else {
				clearInterval(cvi_sztimer);
				if (obj.webkit && rm) {
	...
	_catchKey : function(e) {
		...
		var rm = false, mm = false, k, t, ex, ey, px = 0, py = 0, obj = shiftzoom.G(img.infoid).style;
		e = e ? e : window.event;
		if (e.which) {
			mm = (e.which == 2);
			rm = (e.which == 3);
		...
		if (((e.altKey && !e.shiftKey) || rm || img.zoomout) && !img.automode && !img.nozoom && (img.parentNode.width > img.minwidth || img.parentNode.height > img.minheight)) {
			...
			cvi_sztimer = setInterval("shiftzoom._zoomOut('" + img.id + "'," + rm + "," + (img.webkit && rm ? 1 : 0) + "," + st + "," + sw + "," + ew + "," + sh + "," + eh + "," + sx + "," + ex + ","
					+ sy + "," + ey + "," + img.nozoom + ")", img.millisec);
			...
			cvi_sztimer = setInterval("shiftzoom._zoomIn('" + img.id + "',0," + st + "," + sw + "," + ew + "," + sh + "," + eh + "," + sx + "," + ex + "," + sy + "," + ey + "," + img.nozoom + ")",
					img.millisec);
			...
			if (!img.automode && !img.zoomout && !img.zoomin) {
				...
				if (d == 0 && (parseInt(img.parentNode.style.width) > img.minwidth || parseInt(img.parentNode.style.height) > img.minheight)) {
					...
					if ((cvi_szimage || ww) && (img.parentNode.width > img.minwidth || img.parentNode.height > img.minheight)) {
						if (!ww) {
							setTimeout("shiftzoom._zoomKey(" + d + "," + ct + "," + ww + "," + st + "," + sw + "," + ew + "," + sh + "," + eh + "," + sx + "," + ex + "," + sy + "," + ey + "," + nz
									+ ")", 50);
						} else {
							setoverview();
							if (cvi_szactive != null) {
