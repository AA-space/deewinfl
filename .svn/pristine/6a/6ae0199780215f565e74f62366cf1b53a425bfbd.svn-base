...
(function($) {
	...
	$.Poshytip.prototype = {
		...
		refresh: function(async) {
			...
			this.$tip.css({left: 0, top: 0}).insertAfter(this.$elm);
			...
			//white-space:nowrap;text-overflow:ellipsis;
				this.$tip.css({left: this.pos.l, top: this.pos.t, 'white-space' : 'nowrap', 'text-overflow' : 'ellipsis'});
			}
		},
		...
		calcPos: function() {
			...
			if (this.opts.alignTo == 'cursor') {
				...
				*/
				//计算相对位移
				xL = (this.opts.alignX != 'inner-right' ? 0 : elm.w);	// left edge
				xC = xL + Math.floor(elm.w / 2);				// h center
				xR = (this.opts.alignX != 'inner-left' ? elm.w : 0);	// right edge
				yT = (this.opts.alignY != 'inner-bottom' ? 0 : elm.h);	// top edge
				yC = yT + Math.floor(elm.h / 2);				// v center
				yB = yT + (this.opts.alignY != 'inner-top' ? elm.h : 0);	// bottom edge
			}
			...
			switch (this.opts.alignY) {
				...
				case 'center':
					...
					//else if (pos.t < win.t) //edit by jason
						//pos.t = win.t; //edit by jason
					break;
