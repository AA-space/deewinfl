...
public class IrrServiceByTranImpl {
	...
	/**
	 * 
	 * TODO (todo-list todo-list 调息时会计租金计划irr计算)
	 * 
	 * @param cb
	 * @param tcb
	 * @param frpb
	 ...
	 * @throws Exception
	 */
	public ConditionBean getAdjustFinacIrr(ConditionBean cb, TabCalBean tcb,
			FundRentPlanBean frpb, String calType, String startList)
			throws Exception {
		...
		if (Integer.parseInt(startList) == 1) {// 查看调息是是从第一期开始时则加添加交易结构的现金流明细
			...
			String plan_date = DateTools.dateAdd("month", -12
					/ Integer.parseInt(cb.getIncomeNumberYear()), plan_temp);
		...
		cb.setPlanIrr(new BigDecimal(finacIrr).multiply(new BigDecimal("100"))
				.toString());
