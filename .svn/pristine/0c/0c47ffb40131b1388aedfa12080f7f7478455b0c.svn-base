	...
	for(var i = 0; i < comboboxs.length;i++){
		...
		jQuery.ajax({
			...
			success:function(data){
				...
				createCombobox('#' + type + '_' + tempObj.id,comboData,tempObj.nameField,tempObj.valueField);
			}
...
function createCombobox(targetId,data,nameField,valueField){
...
function loadReportTableData(queryObject){	
	...
	var width = document.documentElement.clientWidth-14;
	var height = document.documentElement.clientHeight - 48;
    ...
    jQuery('#search_' + queryObject.id).css('display','none');
...
function loadReportChartData(queryObject){
	var width = jQuery('#chart_data_' + queryObject.id).width()-2;
	var height = jQuery('#chart_data_' + queryObject.id).parent().height() - 35;
...
function report_openLink(contextPath,type,url,parameters){
	var openUrl = "";
	if(type=="TABLE"){
		...
		openUrl = contextPath + "/report/showPage.action?needjs=1&id=" + url + "&" + parameters;
	}else{
