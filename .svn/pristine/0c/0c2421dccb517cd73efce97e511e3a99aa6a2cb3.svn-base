<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    function setFundFundChargePlanReback(table){
        ...
        for(var i=0;i<plandata.length;i++){
           	 ...
           	 if(tempmessage!=""){
           		 loadMask_oper.hide();
           	    jQuery.messager.alert('提示',"<div style='font-size:18px;line-height:30px;'>金额为"+plandata[i].factmoney+"的保证金"+tempmessage+"不能退款</div>",'error'); 
                 return false;
            }
        	if(checkIdIsAdd(plandata[i].id,iddata)){
                if(isAddPlanData==""){isAddPlanData=plandata[i].feetype+":"+plandata[i].factmoney;}else{isAddPlanData=isAddPlanData+","+plandata[i].feetype+":"+plandata[i].factmoney;}
                 ...
                 tdate.factmoney=parseFloat(tdate.overmoney);
                 var temp=parseFloat(tdate.factmoney)+parseFloat(tdate.feeadjust);
                 tdate.settlemethod="PayFund8";
                 tdate.rawValue_settlemethod="退款";
                 tdate.paytype="pay_type_out";
                 tdate.rawValue_paytype="付款";
                 tdate.ebanknumber="";
                 tdate.oper="old";
                 tdate.pid=tdate.id;
    ...
    jQuery(function(){
	 ...
	 var fund= new tracywindyMultiTable({
	    ...
	    otherTools : [ {
			html : '<font color="red">退款</font>',
			handler : function(table) {
				var fundPlan = table.getCheckedRowDatas();
				if (0 == fundPlan.length) {
				      //alert("请选择需要红冲的保证金计划!");
					 jQuery.messager.alert('提示',"<div style='font-size:18px;line-height:30px;'>请选择需要退款的保证金计划</div>",'error'); 
					return false;
				}
