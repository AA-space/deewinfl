...
(function ($)
{
    ...
    $.ligerDefaults.ComboBox = {
        ...
        onStartResize: null,
        onEndResize: null,
        hideOnLoseFocus: true, //市区焦点的时候隐藏
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        {
            var g = this, p = this.options;
            var value_ = $(this.valueField).val();
            //如果是ajax方式获取数据，因为ajax没有返回值，但已经调用getVavlue那就返回初始化值
            if(value_ == '' && p.initValue != null){
                value_ = p.initValue;
            }
            return value_;
        },
