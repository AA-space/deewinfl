...
<script type="text/javascript">
   ...
   isNeedBankEdit = false;
	...
	function workflowSubmitCallBack()//返回true提交，返回false不提交 ，该回调函数可与用表单字段验证
	{
	    ...
	    for(i=0;i<talbedata.length;i++){
	    	//talbedata[i].factmoney;talbedata[i].paymentid
	    	for(j=0;j<datas.length;j++)
	    	{
		    	if(talbedata[i].paymentid==datas[j].paymentid)
		    	{
			    	if(parseFloat(talbedata[i].factmoney)+parseFloat(talbedata[i].creditmoney)>parseFloat(datas[j].planbalance))
			    	{   
				    	alert("本次投放金额不能大于计划投放金额");
				    	return false;
				    }
			    }
	    	}
		...
		<td colspan=4 class="td-tabs-container" style="text-indent: 0px;">
			...
			<div id='id_project_info_tabs_content' style="">
	               ...
	               <div title="资金收款明细">
					...
					<div title="合同额度明细">
						<span class="print-tabs-title-content">合同额度明细</span>
						<div id="id_table_leasing_contract_limit_list"
							style="overflow: hidden;"></div>
					</div>
...
<jsp:include page="funds_limit_limit.jsp"></jsp:include> <!-- 额度明细 -->
<script language="javascript">
