...
<script type="text/javascript">
	...
	function formPageReadOnlyCallBack(){
	   if(!isCompletedTask){
		     ...
		     jQuery("input[name='framework_condition.equipamt']").attr({
                    readOnly:false
			     }).removeClass("element-readonly");
		     jQuery("input[name='framework_condition.firstpaymentratio']").attr({
                 readOnly:false
			     }).removeClass("element-readonly");
		     jQuery("input[name='framework_condition.gpsmoney']").attr({
                 readOnly:false
			     }).removeClass("element-readonly");
		     jQuery("input[name='framework_condition.insuremoney']").attr({
                 readOnly:false
			     }).removeClass("element-readonly");
		     jQuery("input[name='framework_condition.cautionmoneyratio']").attr({
                 readOnly:false
			     }).removeClass("element-readonly");
		     jQuery("input[name='framework_condition.handlingchargemoneyratio']").attr({
                 readOnly:false
			     }).removeClass("element-readonly");
		     jQuery("input[name='framework_condition.incomenumber']").attr({
                 readOnly:false
			     }).removeClass("element-readonly");
		     jQuery("input[name='framework_condition.nominalprice']").attr({
                 readOnly:false
			     }).removeClass("element-readonly");
		     jQuery("input[name='framework_condition.otherincome']").attr({
                 readOnly:false
			     }).removeClass("element-readonly");
		     getTracywindyObject("id_combo_id_insurer_type").readOnlyData = false;
		     getTracywindyObject("id_combo_id_insurer_type").reload();
		     getTracywindyObject("id_combo_id_period_type").readOnlyData = false;
		     getTracywindyObject("id_combo_id_period_type").reload();
		     getTracywindyObject("id_combo_id_is_registration").readOnlyData = false;
		     getTracywindyObject("id_combo_id_is_registration").reload();
		     getTracywindyObject("id_combo_id_is_guaranty").readOnlyData = false;
	...
	{  
		fillHiddenVal();
		 if(!checkEquipChange()){return  false;};
   ...
   //验证设备款、起租比例等浮动范围
	function checkEquipChange()
	{
        var msg_change="";
        var columnname=["equipamt_wu","firstpaymentratio_wu","cautionmoneyratio_wu","handlingchargemoneyratio_wu","leaseterm_wu"];
        for(var i=0;i<columnname.length;i++)
        {
    		...
    		case "equipamt_wu":
        	    ...
        	    {
    				msg_change+="设备款只能原价10%上下调整！\n";
                }
        		break;
    		case "firstpaymentratio_wu":				
    			if(parseFloat($("#firstpaymentratio_wu").val())>parseFloat($("#firstPaymentRatio").val()))
        		{
    				msg_change+="起租比例只能上调！\n";
    	        }				
    			break;
    		case "cautionmoneyratio_wu":				
    			if(parseFloat($("#cautionmoneyratio_wu").val())>parseFloat($("#cautionMoneyRatio").val()))
        		{
    				msg_change+="保证金比例只能上调！\n";
    	        }	
    			break;
    		
    		case "handlingchargemoneyratio_wu":
    			if(parseFloat($("#handlingchargemoneyratio_wu").val())>parseFloat($("#handlingChargeMoneyRatio").val()))
        		{
    				msg_change+="手续费比例只能上调\n";
    	        }	
    			break;
    		case "leaseterm_wu":
    			if(parseFloat($("#leaseterm_wu").val())<parseFloat($("#incomeNumber").val()))
        		{
    				msg_change+="租赁期限只能下调！\n";
    	        }
        		break;
...
<!--多行控件  -->
<input type="hidden" id="id_json_contract_equip_str_r" name="json_proj_equip_str"  value='${empty json_proj_equip_str ? "[]" : json_proj_equip_str }'></input>
...
<div class="fillTableContainer">
   <table class="fillTable" cellspacing="0" cellpadding="0">
	...
	<!-- 隐藏域，判断修改限定值 -->
	<input name="equipamt_wu" id="equipamt_wu" type="hidden" value="${requestScope['equipamt_wu'] }"/>
	<input name="firstpaymentratio_wu" id="firstpaymentratio_wu" type="hidden" value="${requestScope['firstpaymentratio_wu'] }"/>
	<input name="gpsmoney_wu" id="gpsmoney_wu" type="hidden" value="${requestScope['gpsmoney_wu'] }"/>
	<input name="cautionmoneyratio_wu" id="cautionmoneyratio_wu" type="hidden" value="${requestScope['cautionmoneyratio_wu'] }"/>
	<input name="handlingchargemoneyratio_wu" id="handlingchargemoneyratio_wu" type="hidden" value="${requestScope['handlingchargemoneyratio_wu'] }"/>
	<input name="leaseterm_wu" id="leaseterm_wu" type="hidden" value="${requestScope['leaseterm_wu'] }"/>
	<tr>
		...
		<div id='id_contract_info_tabs_content' style="">
			...
			<script language="javascript">
               jQuery(function() {isViewHistoryTask = false;
				...
				<!--修改前页面:project/proj_approval/proj_equip_detail.jsp-->
				<jsp:include page="contract_equip_detail02-1.jsp"></jsp:include>
...
<script>
	...
	function fillHiddenVal() 
	{
		...
		jQuery('#id_json_contract_equip_str_r').val(JsonUtil.encode(contractEquip.getRowsJsonData()));
	}
