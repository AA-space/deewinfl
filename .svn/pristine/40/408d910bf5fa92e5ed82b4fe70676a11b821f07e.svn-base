...
//	}
	...
	/**
	 * 
	 * TODO (todo-list todo-list 得到变更的租金列表下标值,并以数组返回)
	 * 
	 * @param l_rent_adjust
	 * @return
	 */
	public static String[] getAdjustIds(List l_rent_adjust,String grace) {
		String id_s = "";
		for (int i =Integer.parseInt(grace); i < l_rent_adjust.size(); i++) {
		...
		id_s = id_s.indexOf(",") > -1 ? id_s.substring(0, id_s.length() - 1)
				: id_s;
		if(id_s.equals("")){
			return new String[0];
		}else{
	...
	/**
	 * 
	 * TODO (todo-list todo-list 得到不变的租金列表下标值,并以数组返回)
	 * 
	 * @param l_rent_adjust
	 * @return
	 */
	public static String[] getNoAdjustIds(List l_rent_adjust,String grace) {
		String id_s = "";
		for (int i = Integer.parseInt(grace); i < l_rent_adjust.size(); i++) {
			String adjust = (String) l_rent_adjust.get(i);
			if (adjust.equals("")) {// 当此期租金发生变更时
	...
	/**
	 * 
	 * TODO (todo-list todo-list 调整后的租金值)
	 * 
	 * @param obj
	 * @return
	 */
	@SuppressWarnings("unchecked")
	public static List getAdjustRentList(Object obj) {
		String[] rent_adjust_arr = (String[]) obj;
		List l_rent_adjust = new ArrayList();

		for (int i = 0; i < rent_adjust_arr.length; i++) {
			l_rent_adjust.add(rent_adjust_arr[i]);
		}
		return l_rent_adjust;
	}
