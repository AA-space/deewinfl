//if(window.tracywindyComboBoxCust)alert("不能重复引用tracywindyComboBoxCust.js");
...
var tracywindyComboBoxCust = function(config){
	if(config.id){
   ...
   this.isDisabled=config.isDisabled||false;//默认不可见
   ...
   this.isAsync = config.isAsync || true;
...
tracywindyComboBoxCust.prototype.reload=function(){
...
tracywindyComboBoxCust.prototype.setParam=function(name,value){
    this.params[name]=value;
};
tracywindyComboBoxCust.prototype.setParams=function(params){
...
//创建数据区
tracywindyComboBoxCust.prototype.AddRow=function(rowData){
	this.datas.push(rowData);
	this.loadData();
	return rowData;
};
tracywindyComboBoxCust.prototype.AddRows=function(rowDatas){
...
tracywindyComboBoxCust.prototype.getRowsData=function(){
	return this.datas;
};
tracywindyComboBoxCust.prototype.removeRowAt=function(index){
	var rowData = this.datas.splice(index,1);
	this.loadData();
	return rowData;
};
tracywindyComboBoxCust.prototype.addRowAt=function(index,rowData){
	this.datas.splice(index,0,rowData);
	this.loadData();
	return rowData;
};
tracywindyComboBoxCust.prototype.addRowsAt=function(index,rowDatas){
	if(rowDatas instanceof Array)
	{
...
tracywindyComboBoxCust.prototype.loadAjaxData=function(isAsync){
...
tracywindyComboBoxCust.prototype.createObj=function(){
...
tracywindyComboBoxCust.prototype.setDefaultValue=function(index){
...
tracywindyComboBoxCust.prototype.Offset=function(){
...
tracywindyComboBoxCust.prototype.loadSelect=function(){
	 var domInnerHTML = "";
...
tracywindyComboBoxCust.prototype.setValue=function(value){
	this.showValueDiv.value='';
	this.value = value;
	this.rawValue="";
	this.loadData();
};
tracywindyComboBoxCust.prototype.setFilterValue=function(value,filter){
	this.grantValue = true;
	this.value = value;
	this.rawValue="";
	this.loadData(filter);
};
tracywindyComboBoxCust.prototype.setRawValue=function(rawValue){
	this.showValueDiv.value='';
	this.grantValue = true;
...
tracywindyComboBoxCust.prototype.setFilterRawValue=function(rawValue,filter){
	this.rawValue=rawValue;
	this.value = "";
	this.loadData(filter);
};
tracywindyComboBoxCust.prototype.getValue=function(){
	return this.valueHidden.value;
};
tracywindyComboBoxCust.prototype.getRawValue=function(){
	return this.rawValueHidden.value;
};
tracywindyComboBoxCust.prototype.loadData=function(filter){
      this.lazyLoad = false;
	  //清空数据
	  // var itemHeight = 20;
	   var itemCount = 0;
	   ...
	   for (var i=0;i<datas.length;i++)
	   {
	    ...
	    if(this.isCheck)
	    {
	    	...
	    	if(this.isDisabled){
	    	if(lLi.realValue=='cust_type.cust'&&checkString == "checked"){
	    		disabledString = "disabled";
	    	}
	    	}
	    	lLi.innerHTML="<input type='checkbox' class='x-panel-combo-droplist-li-checkbox' name='dropListLiCheckbox_"+this.id+"' "+checkString+" "+disabledString+" value='"+lLi.realValue+"' rawValue='"+lLi.innerHTML+"'/>"+(data[this.displayField]||'');
	    	//}else{
