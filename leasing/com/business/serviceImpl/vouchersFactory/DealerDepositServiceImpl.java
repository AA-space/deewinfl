...
@Service(value = "dealerDepositService")
public class DealerDepositServiceImpl implements DealerDepositService {
	 ...
	 */
	@Override
	public void collectDepositMoney(MFundFundCharge mc) throws Exception {
		...
		/** 本凭证-公用字段-开始*/
		String workFlowName = "核销逾期保证金"; 
		BigDecimal originalAmount = mc.getFactMoney();                                                                                           //原币金额
		
		String voucherAbstract= "确认"+mc.getDealerId().getCustName()+"逾期保证金";													//摘要
		...
		//建立辅助账
		VoucherassStactsConfig config_01 = config_01 = this.baseService.findEntityByID(VoucherassStactsConfig.class, "225");               //预收账款-客户
		List<IntereasVoucherasStacts> stactsList_01 = voucherService.generateVoucherassStacts(mc.getDealerId(),null, config_01);                                     //辅助账集合
		/** 1 :建立凭证体1-开始 */
...
//		propertiesMap.put("accNumber", fbd.getOwnAccNumber());
		...
		VoucherassStactsConfig config_02 = this.baseService.findEntityByID(VoucherassStactsConfig.class, "231");                          //凭证配置表
		
		//建立辅助账
		
		List<IntereasVoucherasStacts> stactsList_02 = voucherService.generateVoucherassStacts(mc.getDealerId(),null, config_02);                                     //辅助账集合
	 ...
	 */
	@Override
	public void payDepositMoney(MFundFundCharge mc) throws Exception {
		/** 第一步:建立基本凭证体集合和公用字段字段-开始 */
		List<IntereasVoucherHead> headObjList = new ArrayList<IntereasVoucherHead>();                                                                   //凭证头集合
		List<IntereasVoucherEntries> bodyList = Lists.newArrayList();                                                                                   //凭证体集合
		String currentDate = DateUtil.getSystemDate();                                                                                                  // 当前时间
		...
		/** 本凭证-公用字段-开始*/
		//流程名称
		String workFlowName = "支付其他款项"; 
		BigDecimal originalAmount = mc.getFactMoney();                                                                                           //原币金额
		String voucherAbstract = "支付"+mc.getDealerId().getCustName()+"逾期保证金";                     //摘要
		/** 本凭证-公用字段-结束*/
		...
		VoucherassStactsConfig config_01 = this.baseService.findEntityByID(VoucherassStactsConfig.class, "231");   //凭证配置表:其他应付款-逾期保证金
		...
		properMap.put("accNumber",  mc.getAccNumber());
		
		OwnAccount account = this.baseService.findEntityByProperties(OwnAccount.class, properMap).get(0);
		...
		//建立辅助账
		List<IntereasVoucherasStacts> stactsList_02 = voucherService.generateVoucherassStacts(mc.getDealerId(),account, config_02);                                     //辅助账集合
