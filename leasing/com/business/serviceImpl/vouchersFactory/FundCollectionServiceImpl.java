...
public class FundCollectionServiceImpl implements FundCollectionService {
	...
	public void createVoucher(ContractInfo cinfo,BigDecimal sum,FundEbankData fundEbankData) throws Exception {
		...
		String voucherAbstract_01 = "核销"+cinfo.getCustId().getCustName()+cinfo.getContractNumber()+"首付款";//摘要
		...
		//String voucherAbstract_02 = "核销"+cinfo.getCustId().getCustName()+"首付款";//摘要
		...
		if(fundEbankData!=null){
			factdate=fundEbankData.getFactDate();
		} 
	...
	public void createVoucherForFundPenalty(ContractInfo cinfo,BigDecimal sum,FundEbankData fundEbankData) throws Exception {
		...
		VoucherassStactsConfig config_01 = this.baseService.findEntityByID(VoucherassStactsConfig.class, "225");                          //凭证配置表
		String voucherAbstract_01 = "核销"+cinfo.getCustId().getCustName()+"违约金";//摘要
		...
		VoucherassStactsConfig config_02 = this.baseService.findEntityByID(VoucherassStactsConfig.class, "236");                                 //凭证配置表-预收账款
		String voucherAbstract_02 = "核销"+cinfo.getCustId().getCustName()+"违约金";//摘要
	...
	public void createVoucherNominalPrice(ContractInfo cinfo,BigDecimal sum) throws Exception {
		...
		*//************************************** 凭证体1-分割线-开始 ********************************************************//*
		...
		VoucherassStactsConfig config_01 = this.baseService.findEntityByID(VoucherassStactsConfig.class, "229");                          //凭证配置表
		String voucherAbstract_01 = "确认‘"+cinfo.getCustId().getCustName()+"’-留购价收入‘"+Tools.formatNumberDoubleTwo( String.valueOf(originalAmount) )+"’";//摘要
		...
		*//** 主营业务收入--留购款收入*//*
		*//************************************** 凭证体2-分割线-开始 ********************************************************//*
		BigDecimal nominalIncome=originalAmount.divide(new BigDecimal(1.17),2,BigDecimal.ROUND_HALF_EVEN);
		VoucherassStactsConfig config_02 = this.baseService.findEntityByID(VoucherassStactsConfig.class, "235");                                 //凭证配置表-主营业务收入--留购款收入
		String voucherAbstract_02 = "确认‘"+cinfo.getCustId().getCustName()+"’-留购款收入‘"+Tools.formatNumberDoubleTwo( String.valueOf(nominalIncome) )+"’";//摘要
		...
		*//**
		...
		//构建凭证体1需要的参数:
		body_02.setAccountNumber(config_02);                       //这个科目对象
		body_02.setOriginalAmount(nominalIncome);               //原币金额 
		body_02.setEntryDC(CR);                                    //借贷方向 （DR(1) 借方- CR(1) 贷方）
		body_02.setVoucherAbstract(voucherAbstract_02);            //摘要
		body_02.setCurrencyNumber(currencyNumber);                 //币别数据字段对象
		body_02.setIntereasVoucherasStactsInAction(stactsList_02); //保存辅助账
		...
		*//** 应交税费-应交增值税-销项税额*//*
		*//************************************** 凭证体3-分割线-开始 ********************************************************//*
		BigDecimal nominalIncomeTax=nominalIncome.multiply(new BigDecimal(0.17)).setScale(2, BigDecimal.ROUND_HALF_UP);
		VoucherassStactsConfig config_03 = this.baseService.findEntityByID(VoucherassStactsConfig.class, "238");                                 //凭证配置表-应交税费-应交增值税-销项税额
		String voucherAbstract_03 = "确认‘"+cinfo.getCustId().getCustName()+"’-留购款收入‘"+Tools.formatNumberDoubleTwo( String.valueOf(nominalIncomeTax) )+"’";//摘要
		...
		*//**
		...
		body_03.setOriginalAmount(nominalIncomeTax);               //原币金额 
