package com.tenwa.leasing.rentcalc.test;
...
public class Tools {
	...
	public static void handSt(String contract_id, String doc_id)
		...
		if (doc_id.equals("")) {
			...
			List<Map<String, String>> rs_list = db.executeQuery(sqlstr);
			for(Map<String, String> rs:rs_list){
				start_list = getDBStr(rs.get("rent_list"));
			}
			sqlstr = "select * from fund_rent_plan where contract_id='"
					+ contract_id
					+ "' and plan_status='未回笼' order by rent_list";
			rs_list = db.executeQuery(sqlstr);
			for(Map<String, String> rs:rs_list){
				l_interest.add(getDBStr(rs.get("interest")));
			}
			...
			for (int i = 0; i < l_st.size(); i++) {
				sqlstr = "update fund_rent_plan_temp set straight_interest="
						+ l_st.get(i) + " where contract_id='" + contract_id
						+ "' and doc_id='" + doc_id + "' and rent_list="
						+ (Integer.parseInt(start_list) + i);
				db.executeUpdate(sqlstr);
			}
			db.close();
	...
	public static void main(String[] args) {
		...
		System.out.println(st.substring(st.indexOf("-") + 1, st
				.lastIndexOf("-")));

		System.out.println(Tools.dateAdd("month", 1, st));

		System.out.println(getLastDayOfMonth("2009", "8") + "=="
				+ st.substring(st.lastIndexOf("-") + 1, st.length()));
	}
}
