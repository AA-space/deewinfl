...
public class InterestTools {
	 ...
	 * @param cb 商务条件
	 * @param frpb 本次计算出来的租金计划数据
	 * @param frpb_old 本次计算前的租金计划数据
	 * @param startList 本次计算租金的开始期项
	 * @return 返回全部的手续分摊情况
	 */
	public List<String> getHandlingChargeMoneyShare(ConditionBean cb,FundRentPlanBean frpb,FundRentPlanBean frpb_old,  int startList){
		...
		BigDecimal sum_corpus=new BigDecimal( cb.getCalTotalByCont());
		if(handling_charge_money_total.compareTo(BigDecimal.ZERO)==0){//偷懒做法,如果没有手续费就返回0
			...
			int grace=Integer.parseInt(cb.getGrace());//宽限期
			//按照原有的租金计划计算本次手续费分摊,只有在不是第一次测算时才有原租金计划 所以需要判断
			if(frpb_old!=null&&frpb_old.getCorpusOverage_list().size()>0){
				corpus_overage=new ArrayList<String>(frpb_old.getCorpusOverage_list());
			}else{
				corpus_overage=new ArrayList<String>(frpb.getCorpusOverage_list());
			}
			...
			if(startList!=1){//如果不是第一期开始的就需要计算剩余的重新分摊
				...
				//从开始期数计算本金总和
				sum_corpus=new BigDecimal(NumTools.getSumCorpusOverage(frpb.getCorpus_list(), startList));
				//按照新的条件计算手续费分摊数据
	...
	public static List<String> getHandlingChargeMoneyShare(BigDecimal hand_change_money,BigDecimal sum_corpus,List<String> corpus_overage,int grace,int period_type){
		...
		for(int i=0;i<corpus_overage_temp.size();i++){
			...
			if(i!=corpus_overage_temp.size()){//按照比例计算手续费分摊值
				temp_money=hand_change_money.multiply(this_corpus_overage.divide(sum_corpus_overage,20,BigDecimal.ROUND_HALF_UP))
	 ...
	 * @param handling_charge_money 总手续费
	 * @param interest_list 财务租金计划-利息
	 * @param finac_interest_list 会计租金计划-利息
	 * @param end_list 截止期数
