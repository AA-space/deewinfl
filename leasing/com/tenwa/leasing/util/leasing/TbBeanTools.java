 ...
 * @date 2011-3-3
 * @desc TODO (todo-list 此类用于根据流程类型得出，交易结构，租金计划所用的表信息)
 */
public class TbBeanTools {
	...
	/**
	 * 
	 * TODO (todo-list todo-list 根据交易结构信息，测算类型得到测算所涉及的表信息)
	 * 
	 * @param calType
	 * @param cb
	 * @return
	...
	public static TabCalBean getTabInfo(String calType, ConditionBean cb) throws Exception {
		...
		tcb.setDocIdCoulValue(cb.getDocId());
		if(cb.getOnhireId()!=null&&!cb.getOnhireId().equals("")){
			tcb.setOnHire_id(cb.getOnhireId());
		}
		tcb.setId_1(cb.getId_1());
		tcb.setId_2(cb.getId_2());
		...
		if("onHire_more_process".equals(calType)&&!cb.getLastCorpus().equals("")){
			tcb.setIs_merger("是");
		}else{
			tcb.setIs_merger("否");
		}
		tcb.setUserId(cb.getCreator());//登陆人
		tcb.setQuot_id(cb.getQuotId());//报价编号 
	...
	/**
	 * 
	 * TODO (todo-list todo-list 根据变更bean信息，测算类型得到测算所涉及的表信息)
	 * @param calType
	 * @param ajb
	 * @return
	 * @throws Exception
	...
	public static TabCalBean getTabInfo(String calType,AdjustBean ajb) throws Exception {
		...
		tcb.setDocIdCoulValue(ajb.getMeasureId());
