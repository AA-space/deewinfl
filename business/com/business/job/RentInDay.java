	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		...
		try {
			...
			String sql = "SELECT pln.id FROM Contract_Fund_Rent_Plan pln left join contract_info c on pln.contract_id=c.id "
					+ " WHERE pln.plan_Date= '"+currentDate+"' and c.contract_Status not in  ('71','111')  "
					+ " and pln.CONTRACT_ID not in (select a.contract_id from VI_CONTRACT_PENALTY  a"
					+ " left join VI_CONTRACT_OUT_LIST out on a.CONTRACT_ID=out.CONTRACT_ID "
					+ " where penalty >0 and out.out_list>=3)" ;
			
			List<String> rentPlanIds = new LinkedList<String>();
			...
			List<String> list_ad = new LinkedList<String>();
