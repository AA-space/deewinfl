<%@ page language="java" contentType="text/html; charset=UTF-8"
                  ...
                  if(tabledata[i].rent<0||tabledata[i].penalty<0){
                      if(ebankId==tabledata[i].ebanknumber){
                    	  ...
                    	  if(parseFloat(fundList[j].corpusov)>0){
	                  	  ...
	                  	  if(parseFloat(fundList[j].penaltyov)>0){
                	  ...
                	  ebankMoney=parseFloat(parseFloat(ebankMoney)-parseFloat(tabledata[i].corpus)-parseFloat(tabledata[i].interest)-parseFloat(tabledata[i].penalty)).toFixed(2);
	                  if(parseFloat(fundList[j].corpusov)>0){
	              	  		fundList[j].corpusov=parseFloat(fundList[j].corpusov)-parseFloat(tabledata[i].corpus);
	              	  }
	              	  if(parseFloat(fundList[j].interestov)>0){
	...
	function comptuerCurMoney(){
	    ...
	    var temp=parseFloat(parseFloat(corpus)+parseFloat(interest)+parseFloat(penalty)+parseFloat(interestad)+parseFloat(penaltyad)+parseFloat(interestadjust)+parseFloat(penaltyadjust)).toFixed(2);
        $("#re_receivermoney").val(temp);
        var _index=parseFloat(parseFloat(corpus)+parseFloat(interest)+parseFloat(interestad)).toFixed(2);
        $("#re_rent").val(_index);
		}
	var oldIsViewHistoryTask = isViewHistoryTask;
	jQuery(function(){
	...
	var table_fund_rent_inComes = new tracywindyMultiTable({
	    ...
	    columns:[
	       ...
	       {name:'ebanknumber',  header:'网银编号',readOnly:false},
	       ...
	       {name:'corpus',  header:'回笼本金' , nullable:false,type:'double', otherAttributes:'id="re_corpus" onkeyup="comptuerCurMoney()"'},
	       {name:'interest',  header:'回笼租息', nullable:false,type:'double', otherAttributes:'id="re_interest" onkeyup="comptuerCurMoney()"'},
	       {name:'penalty',  header:'回笼罚息' , nullable:false,type:'double', otherAttributes:'id="re_penalty" onkeyup="comptuerCurMoney()"'},
	       {name:'interestadjust',  header:'租息调整', nullable:false,type:'double', otherAttributes:'id="re_interestadjust" onkeyup="comptuerCurMoney()"'},
	       {name:'penaltyadjust',  header:'罚息调整', nullable:false,type:'double', otherAttributes:'id="re_penaltyadjust" onkeyup="comptuerCurMoney()"'},
	       ...
	       {name:'ownbank',  header:'本方银行',readOnly:true},
	       {name:'ownaccount',  header:'本方银行账户',readOnly:true},
	       {name:'accnumber',  header:'本方银行账号',readOnly:true},
	...
	isViewHistoryTask = oldIsViewHistoryTask;
});
	</script>