...
jQuery(function(){
	...
	var all_height = document.body.clientHeight;
	 combo_crediter = new tracywindyComboBox({
	    ...
	    params:{
	       pid:'cust_type'
			...
			onSelect:function(combo){
				var crediter_name=combo.getRawValue();
				var combo_creditId = getTracywindyObject("id_combo_creditId");
				combo_creditId.setParams({"crediter":crediter_name});
				combo_creditId.reload();
	    	}
	 });
	 combo_creditId = new tracywindyComboBox({
	    id:'id_combo_creditId',
	    renderTo:'id_creditId',
	    xmlFileName:'\\eleasing\\jsp\\financing_config\\finacing_config_creditId.xml',
	    loadMode:'ajax',
	    readOnly:true,
		isContainEmpty:false,
	 ...
	 onSelect:function(combo,comboData){
	    }
	 });
	...
	function openWindow(){
		var credit_id = getTracywindyObject('id_combo_creditId').getValue();
		//var creditId = getTracywindyObject('id_combo_creditId').getRawValue();
		alert(credit_id);
		if(!credit_id) {
			alert("请选择授信标号!");
			return false;
		}else {
			   var attachmentParams="credit_id="+credit_id;
		       startProcess("申报流程-1",attachmentParams);
	       }
	}
</script>
</head>
