...
<title>项目信审 - 07项目信审审批（只读）-董事会</title>
...
<body>
	...
	<div class="mini-panel" title="项目商务条件" maskOnLoad="false" showCollapseButton="true" style="width: 100%;height:auto;">
			<div class="mini-tabs" activeIndex="0" plain="false" style="width: 100%;height:500px;" id="tab1" onactivechanged="changeheight">
			    ...
			    <div title="信审会记录" name="letterDetail"   iconCls="icon-cut" url="<%=request.getContextPath()%>/workflow/forms/project_nocar/proj_credit_nocar/proj_letter_record.bi?isViewHistoryTask=true">
			        
			    </div>
			    <div title="董事会记录" name="directorDetail"   iconCls="icon-cut" url="<%=request.getContextPath()%>/workflow/forms/project_nocar/proj_credit_nocar/proj_director_record.bi">
...
//         }, 2000);
    ...
    function fillHiddenVal(type){
    	...
    	var Tab8 = tabs.getTab("directorDetail");
    	...
    	var iframe8 = tabs.getTabIFrameEl(Tab8);
...
//     	}
    	...
    	if(typeof(iframe8)!='undefined'){
    		var grid3 = iframe8.contentWindow.returnTab();
    		if("submit"==type){
    		    grid3.validate();
    		    if (grid3.isValid() == false) {
    			    alert("董事会记录错误！！");
    		...
    		jQuery('#id_json_proj_director_detail_str').val(JsonUtil.encode(grid3.getData()));
    	}
