...
jQuery(function(){
	//dictYesOrNoByCode('id_isAffiliated','isAffiliated',1,'','','require="true" label="是否挂靠"');
		...
		/*    {name:'equiptypename', header:'车辆类别',type:'combobox',readOnly:true,nullable:false,hiddenName:'equiptype',
		   ...
		   {name:'modelname', header:'规格型号',type:'combobox',readOnly:false,nullable:true,hiddenName:'model',
			    config:{
				       width:165,
		   ...
		   {name:'chassisnum',  header:'车架号',otherAttributes:'maxB="100"',otherAttributes:'id="id_chassisnum" onblur="checkchassnum2();"',nullable:false},
		...
		function checkChassisnum(op)
		{
			return 	op.match(new RegExp("^[A-Za-z0-9]+$"));
		}
		function checkchassnum2()
		{
			 var chassisnum = $("#id_chassisnum").val();
			 if ( chassisnum != "")
			 {
				 if (!checkChassisnum(chassisnum) || chassisnum.length < 7 )
				 {
				 	alert("车架号只能是数字和字母,长度不能小于7位");
				 	$("#id_chassisnum").val("");
				 	return false;
				 }
			 }
