...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
    ...
    <input name="proj_info.twolevelstatus" id="proj_info.twolevelstatus" type="hidden" value="${requestScope['proj_info.twolevelstatus'] }"/>
    <input name="cust_info.dealername" id="cust_info.dealername" type="hidden" value="${requestScope['cust_info.dealername'] }"/>
    <input name="proj_info.deptid" id="proj_info.deptid" type="hidden" value="${requestScope['proj_info.deptid'] }"/>
    <input name="proj_info.loginusername" id="proj_info.loginusername" type="hidden" value="${requestScope['proj_info.loginusername']}"/>
    ...
    <table cellspacing="0" cellpadding="0"  id="proj_base_info">
         ...
         <tr class="tr-project-info tr-odd" id="id_business_table">
           ...
           <tr class="tr-project-info tr-even">
             ...
             <td class="td-content-title">历史交易记录：</td><td class="td-content"><input name="proj_info.prohistory" id ="project_prohistory"  class="td-content-input td-content-readonly" readOnly type="text" value="${requestScope['proj_info.prohistory']}"></td>
          ...
          <tr class="tr-project-info tr-odd">   
             <td class="td-content-title">项目出单部门：</td><td class="td-content"><input id="projdept" name="proj_info.projdept" class="td-content-input td-content-readonly" readOnly require="true"  type="text" value="${requestScope['proj_info.projdept'] }" ></td>
             <td class="td-content-title">录入人员：</td><td class="td-content"><input id="projregistrar" name="proj_info.projregistrar" class="td-content-input td-content-readonly" readOnly require="true"  type="text" value="${requestScope['proj_info.projregistrar'] }" ></td>
         </tr>
          <tr class="tr-project-info tr-even">   
             <td class="td-content-title">项目经理：</td><td class="td-content" colspan="3">
        ...
        jQuery("#id_business_table").hide();//不可见,不需要选择
		    ...
		    jQuery("#proj_base_info").css("width",formWidth+"px");
		        jQuery(function(){
			        ...
			        dict('id_proj_type','proj_info.projtype','proj_type','${requestScope["proj_info.projtype"]}','非关联','require="true" label="活动类型"'); 
			        ...
			        dict('id_leas_form','proj_info.leasform','leas_form','${empty requestScope["proj_info.leasform"] ? "lease_form2" : requestScope["proj_info.leasform"]}',null,'require="true" label="租赁形式"');
			        ...
			        jQuery("#projdept").val("${requestScope['cust_info.dealername'] }");
			        jQuery("#projregistrar").val("${requestScope['cust_info.dealername'] }");
          ...
          <!-- 插入法人客户 -->
         <jsp:include page="${requestScope['proj_info.cust_type']=='法人' ?'proj_law_cust_info.jsp':'proj_cust_info.jsp'}"></jsp:include>
