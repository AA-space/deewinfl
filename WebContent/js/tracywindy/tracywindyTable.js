...
function expandOrCollapseQueryArea(tableId) {
...
var tracywindyTable = function(config) {
    ...
    this.showQueryAreaOnDocumentReady = (config.showQueryAreaOnDocumentReady==true)?true:false;
    //创建table
...
tracywindyTable.prototype.createQueryArea = function() {
    ...
    for (var i = 0; i < columns.length; i++) {
        ...
        if (!isHidden && columnQueryConfig) {
            ...
            case "text":
                {
                    valueContent = '<input type="text"  name="' + inputName + '" class="td-content-input"/>';
                    break;
                }
            case "textarea":
                {
            ...
            case "number":
                {
                    ...
                    valueContent = '<input type="text" name="' + inputName + '" class="td-content-input" ' + otherAttributes + ' />';
                    break;
                }
            case "double":
                {
            ...
            case "date":
                {
                    ...
                    if (columnConfig['isRange']) {
                        valueContent = '<div style="width:400px;"><span style="float:left;">&nbsp;从&nbsp;</span><input readonly type="text" name="' + inputName + '_start" class="Wdate td-content-input' + otherClasses + '" onClick="WdatePicker(this,{readOnly:true,dateFmt:\'' + dateFormat + '\'})" ' + otherAttributes + ' />';
                        valueContent += '<span style="float:left;">&nbsp;到&nbsp;</span><input readonly type="text" name="' + inputName + '_end" class="Wdate td-content-input' + otherClasses + '" onClick="WdatePicker(this,{readOnly:true,dateFmt:\'' + dateFormat + '\'})" ' + otherAttributes + ' />';
                    }
                    break;
                }
            case "combobox":
                {
                    ...
                    //模拟下拉框
                    valueContent += '<div id="' + comboboxDivContianerId + '"></div>';
...
tracywindyTable.prototype.load=function(){
	this.params['start'] = '0';
	this.params['limit'] = this.pageSize||20;
...
tracywindyTable.prototype.createTableObj = function() {
    ...
    if (this.title) {
        ...
        if (this.isExcel) {
            titleInnerHTML += "<span id='" + this.id + "_operSpan' class='x-panel-table-div-title-operSpan' title='导出excel' onclick='exportExcelData(\"" + this.id + "\")'>&nbsp;</span>";
        }
...
function doClickTableRow($me, tr, rowIndex) {
    if ($me.onRowClicked) {
        $me.onRowClicked($me, tr, rowIndex);
    }
    if ($me.isChecked && (!$me.checkOnly)) {
...
function createAllDataTable($me, complexHeaders, tableHeaderDivContainer, tableHeaderDiv, tableDiv, table, flag) {
    ...
    for (var index = 0; index < tempDatasLen; index++) {
        ...
        var clickHandler = (function($me, tr, rowIndex) {
            return function() {
                doClickTableRow($me, tr, rowIndex);
            };
        ...
        for (var ccIndex = 0; ccIndex < tableItemColumnLen; ccIndex++) {
            ...
            var tempInnerHTML = "";
            if (((tempDatasLen - 1) == index) && ($me.statisticColumnNames.length > 0)) {
...
function createPage($me) {
    var jsonData = $me.jsonData;
    var totalCountAttibute = $me.basicInfo.totalCount;
    var totalCount = $me.totalCount || jsonData[totalCountAttibute];
    var page = $me.pageDiv;
    var start = parseInt($me.params['start'] || $me.start);
    var pageSize = parseInt($me.params['limit'] || $me.pageSize);
    var realTotalCount = parseInt(totalCount);
...
function goPage(tableId, toWhere) {
    ...
    var totalCount = currentTable.realTotalCount;
    ...
    if ('local' == currentTable.loadMode) {
        ...
        currentTable.params['start'] = start;
        currentTable.params['limit'] = pageSize;
        currentTable.params['totalCount'] = totalCount;
        currentTable.loadAjaxTableData();
    }
}
function doLocalTablePageReload(params, start, limit) {
    ...
    jsonObj['table_load_local_page_limit'] = limit;
