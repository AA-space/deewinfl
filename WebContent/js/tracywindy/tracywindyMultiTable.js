var tracywindyMultiTable = function(config){
			 	   	   ...
			 	   	   case "combobox":{
			 	   		   ...
			 	   		   var selectFunc  = (function($$me,columnConfig,config){
			 	   			   return function(combo,rowData){
				 	   			   ...
				 	   			   //增加下拉列表名称保存单独处理----rovine 20150914
				 	   			   var titilname=this.getRawValue();
				 	   			   if(columnConfig['splitname'])
				 	   			   {
				 	   				  titilname=titilname.substring(0, this.getRawValue().indexOf(columnConfig['splitname']));
				 	   			   }
				 	   			
				 	   			   jQuery("#"+$$me.windowId+" form input[name='"+columnConfig['name']+"']").val(titilname);
					 	   		   /*cascade:{
					 	   			  parentColumnNames:[],
					 	   			  childrenColumnNames:[]
					 	   		   }*/
	...
	*/
		         ...
		         var clickFunc = (function($me,currentTable){
		        	 return function(e){
		        		 ...
		        		 for(var ci=0;ci<copyCount;ci++){
		    			   ...
		    			   {
		    				  var newRowData = {};
		    				  for(var p in rowDatas[i])
		    				  {
		    					 newRowData[p] = rowDatas[i][p];
		    				  }
		    				  newRowData[$me.idField] = '';
		    				  newRowDatas.push(newRowData);
		    				  if(config.copyOperChangeDataCallBack)config.copyOperChangeDataCallBack($me,currentTable,newRowData);
		    			   }
	...
	this.removeOperCallBack = function($me,currentTable){
		...
		if(config.addOperCallBack)config.addOperCallBack($me,currentTable);
	};
	var $toolsMapping = {
			"新增":{
