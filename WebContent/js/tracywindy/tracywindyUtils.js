...
function attachmentUp(config){
	...
	if(document.getElementById("id_downOrLoadFormWindow")){
		var trnode=document.getElementById("id_downOrLoadFormWindow"); 
		trnode.parentNode.removeChild(trnode);
	}
	 	   var uploadAttachmentFileWindow_html = "";
	 	   uploadAttachmentFileWindow_html+='<div id="id_downOrLoadFormWindow"  closed="true" modal="true" title=\"'+config.title+'\" style="display:none;width:300px;text-align:center;background-color:#FDF9F8;">';
	 	   ...
	 	   uploadAttachmentFileWindow_html+='		            <tr><td colspan=2><textarea id="id_fileids" name="ids" style="display:none"></textarea> </td></tr>';
	 	   ...
	 	   uploadAttachmentFileWindow_html+='					        <a style="margin-left:20px;" href="javascript:void(0);" class="btn btn-primary"  onclick=\'javascript:jQuery("#id_downOrLoadFormWindow").window("close");\'><span>取消</span></a>';
    ...
    if(config.ids){
        $("#id_fileids").val(JsonUtil.encode(config.ids));
      }
...
function attachmentUp3(config){
	...
	uploadAttachmentFileWindow_html += '					<tr class="content-separator">';
	uploadAttachmentFileWindow_html += '						<td colspan="2">';
	uploadAttachmentFileWindow_html += '							<a  class="mini-button" iconCls="icon-add" onclick="javascript:attachmentFormSubmit3(&quot;'+getRootPath()+openurl+'&quot;);"><span>确定</span></a>';
...
function attachmentFormSubmit3(url){
...
function attachmentDown(config){
	...
	if(config.returnType=="file"){
        ...
        var param=fileDownLoadParams(config,"url,title,parames,ids","strs");
...
function attachmentDown2(config){
		     ...
		     success:function(rs){
     		   ...
     		   var contractlist= rs.responseText;
	               ...
	               window.currentImportExcelLoadMask.hide();
	   	         ...
	   	         if(vlist.result=="success"){
     		      if(tempreturnType=="listtocurpaget"){
     		    	  	var list = mini.get("reportxlsgrid");
     	   	            list.addRows(vlist.message,0);
     	   	            //parent.document.getElementById("id_json_word_list_str").value=mini.encode(list.getData());
	   		         }else{
	   		        	 window.open(getRootPath()+'/leasing/common/templateManager/list.bi?id='+vlist.message[0].id,'newfilelist','height=300,width=800,top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
     		      }
	   	         }else{
...
function fileDownLoadParams(config,exectFeild,reType){
	...
	var parames="";
	var vparames={};
	var array=exectFeild.split(",");
	for(var field in config){
	   if(parames!=""){parames=parames+"&";}
	   if($.inArray(field, array)>=0){}else{
    ...
    if(reType=="array"){return vparames;}
    else{return parames;}
...
//同步删除某个多行中PID为定的值的多行
function deleteDeduction(PID,tableid){
	  var tableidO='table_'+tableid;
	  var planObject=getTracywindyObject(tableidO);
	  var planData=planObject.getRowsData();
	  for(var i=0;i<planData.length;i++){
	         if(planData[i].pid==PID){
	        	 var removeIndex = planData[i].rowIndex ;
	        	 planObject.removeRowAt(removeIndex); 
	         }
	  }	
}
//检查抵扣资金是否相等
function checktableDeducation(paytalbeid,chargetalbeid){
	var payObject=getTracywindyObject(paytalbeid);
	var chargeObject=getTracywindyObject(chargetalbeid);
	var payPlan=payObject.getRowsData();
	var chargePlan=chargeObject.getRowsData();
	for(var i=0;i<payPlan.length;i++){
		//if(payPlan[i].rawValue_settlemethod=="抵扣"){
			var id=payPlan[i].id;
			var pid=payPlan[i].pid;
			var cmoney=payPlan[i].factmoney;
			var flag=false;
			for(var j=0;j<chargePlan.length;j++){
				//alert(chargePlan[j].id+"/"+pid+","+chargePlan[j].pid+"/"+id+","+parseFloat(chargePlan[j].factmoney)+"/"+parseFloat(cmoney));
				if(chargePlan[j].id==pid&&chargePlan[j].pid==id&&parseFloat(chargePlan[j].factmoney)==parseFloat(cmoney)){
				 flag=true;	
				}
			}
			if(!flag){
				alert(payPlan[i].rawValue_feetype+"["+cmoney+"]抵扣和实际的不相等");
				return false;
			}
		//}
	}
...
//检查抵扣租金是否相等
function checktableDeducationRent(paytalbeid,chargetalbeid){
	var payObject=getTracywindyObject(paytalbeid);
	var chargeObject=getTracywindyObject(chargetalbeid);
	var payPlan=payObject.getRowsData();
	var chargePlan=chargeObject.getRowsData();
	for(var i=0;i<payPlan.length;i++){
		//if(payPlan[i].rawValue_settlemethod=="抵扣"){
			var id=payPlan[i].id;
			var pid=payPlan[i].pid;
			var cmoney=parseFloat(payPlan[i].rent)+parseFloat(payPlan[i].penalty);
			var flag=false;
			for(var j=0;j<chargePlan.length;j++){
				//alert(chargePlan[j].id+"/"+pid+","+chargePlan[j].pid+"/"+id+","+parseFloat(chargePlan[j].factmoney)+"/"+parseFloat(cmoney));
				if(chargePlan[j].id==pid&&chargePlan[j].pid==id&&parseFloat(chargePlan[j].factmoney)==parseFloat(cmoney)){
			...
			if(!flag){
				alert("租金第"+payPlan[i].planlist+"期金额为["+cmoney+"]和投放抵扣的不相等");
				return false;
			}
		//}
	}
