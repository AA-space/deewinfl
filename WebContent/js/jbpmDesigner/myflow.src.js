(function (b) {
    ...
    a.rect = function (p, m) {
        ...
        b([t.node, f.node, n.node, e.node]).bind("click", function (e) {
            if (!a.config.editable) {
            	if(window.globalWorkflowNodeClickCallBack){var nodeType=  (n.node.string||($(n.node.childNodes[0]).text())).replace(/(<<)|(>>)/gi,"");var nodeName = f.node.string||($(f.node.childNodes[0]).text());var posX = parseInt(t.X||(t.node.getAttribute("x")));var posY = parseInt(t.Y||(t.node.getAttribute("y")));var nodeWidth=parseInt(t.W||(t.node.getAttribute("width")));var nodeHeight = parseInt(t.H||(t.node.getAttribute("height")));window.globalWorkflowNodeClickCallBack(e,nodeType,nodeName,posX,posY,nodeWidth,nodeHeight);}
                return
            }
            w();
            var o = b(C).data("mod");
