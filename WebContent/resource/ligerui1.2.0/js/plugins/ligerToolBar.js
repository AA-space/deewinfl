...
(function ($)
{
    ...
    $.ligerui.controls.ToolBar.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _setItems: function (items)
        {
            ...
            if(g.rightItem.length > 0){
            	g.rightItem.reverse();
            	for(var int = 0; int < g.rightItem.length; int++){
            		g.rightItem[int].addClass('l-toolbar-item-right');
            	}
            }
        },
        ...
        addItem: function (item)
        {
			...
			if (item.right)
			{
				g.rightItem.push(ditem);
			}
			if(item.render){
				item.text &&  $("span:first", ditem).html(item.render);
				var button = $("button", ditem);
				item.click && button.click(function () {if ($(this).hasClass("l-toolbar-item-disable")) return;
				var inputVal = $("input", ditem).val();
				var selectVal = $("select", ditem).val();
				item.click(item, selectVal, inputVal);});return;
			}
