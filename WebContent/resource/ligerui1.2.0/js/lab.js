...
{
	if(data){
		var jsonData = lab.valueToJson(data);
		console.info(jsonData);
		$(":input", mainform).each(function(i, v){
			var ligeruiid = v.getAttribute('ligeruiid')
			if(ligeruiid){
				var g = $.ligerui.get(v.getAttribute('ligeruiid'));
				var value = jsonData[v.name];
				if(value && typeof value == 'object' && value.id){
					g.setValue(value.id);
				} else{
					g.setValue(data[v.name]);
				}
			} 			
